...
#define CHIEFTAVISH		"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Chief Tavish",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Chief Tavish from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFTAVISH[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechieftavish_version", PLUGIN_VERSION, "[TF2] Be the Chief Tavish version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechieftavish", Command_ChiefTavish, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefTavishSH);
...
{
//	CreateNative("BeChiefTavish_MakeChiefTavish", Native_SetChiefTavish);
//	CreateNative("BeChiefTavish_IsChiefTavish", Native_IsChiefTavish);
...
{
	if (g_bIsCHIEFTAVISH[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFTAVISH[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFTAVISH);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFTAVISH[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefTavish(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeChiefTavish(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 54825.0);
...
{
	if (IsValidClient(client))
			GiveChiefTavish(client);
}
 
stock GiveChiefTavish(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFTAVISH[client] = true;
		...
		CreateWeapon(client, "tf_wearable", 405, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		CreateWeapon(client, "tf_wearable_demoshield", 131, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		CreateWeapon(client, "tf_weapon_sword", 132, 6, 1, 2, 0);
		
		CreateHat(client, 342, 10, 6);
		CreateHat(client, 874, 10, 6);
		CreateHat(client, 30073, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFTAVISH[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:ChiefTavishSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFTAVISH[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefTavish(Handle:plugin, args)
        MakeChiefTavish(GetNativeCell(1));
 
public Native_IsChiefTavish(Handle:plugin, args)
        return g_bIsCHIEFTAVISH[GetNativeCell(1)];*/

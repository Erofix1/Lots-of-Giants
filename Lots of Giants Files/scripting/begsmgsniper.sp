...
#define GSMGSNIPER		"models/bots/sniper/bot_sniper.mdl"
...
{
	name = "[TF2] Be the Giant SMG Sniper",
	author = "Erofix using the code from: HiGPS and his Friends, Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant SMG Sniper from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGSMGSNIPER[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begsmgsniper_version", PLUGIN_VERSION, "[TF2] Be the Giant SMG Sniper version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begsmgsniper", Command_GiantSmgSniper, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantSmgSniperSH);
...
{
//	CreateNative("BeGiantSmgSniper_MakeGiantSmgSniper", Native_SetGiantSmgSniper);
//	CreateNative("BeGiantSmgSniper_IsGiantSmgSniper", Native_IsGiantSmgSniper);
...
{
	if (g_bIsGSMGSNIPER[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGSMGSNIPER[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GSMGSNIPER);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGSMGSNIPER[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantSmgSniper(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantSmgSniper(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant SMG Sniper !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant SMG Sniper !");
		PrintToChat(target_list[i], "2. Your SMG fires more bullets, deals more damage and has a bigger clip size.");
...
{
	if (IsValidClient(client))
			GiveGiantSmgSniper(client);
}
 
stock GiveGiantSmgSniper(client)
{
	if (IsValidClient(client))
	{
		g_bIsGSMGSNIPER[client] = true;
		...
		CreateWeapon(client, "tf_weapon_smg", 16, 6, 1, 2, 0);
		...
		if(IsValidEntity(Weapon2))
		{
			...
			TF2Attrib_SetByName(Weapon2, "damage bonus", 2.0);
			TF2Attrib_SetByName(Weapon2, "bullets per shot bonus", 3.0);
			TF2Attrib_SetByName(Weapon2, "reload time increased", 1.5);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGSMGSNIPER[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
		TF2Attrib_RemoveByName(Weapon2, "damage bonus");
		TF2Attrib_RemoveByName(Weapon2, "bullets per shot bonus");
		TF2Attrib_RemoveByName(Weapon2, "reload time increased");
		TF2Attrib_RemoveByName(Weapon2, "clip size bonus");
		TF2Attrib_RemoveByName(Weapon2, "maxammo secondary increased");	
		TF2Attrib_RemoveByName(Weapon2, "killstreak tier");	
	}
}
 
public Action:GiantSmgSniperSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGSMGSNIPER[entity]) return Plugin_Continue;
	...
	ReplaceString(sample, sizeof(sample), "_", "_", false);
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantSmgSniper(Handle:plugin, args)
        MakeGiantSmgSniper(GetNativeCell(1));
 
public Native_IsGiantSmgSniper(Handle:plugin, args)
        return g_bIsGSMGSNIPER[GetNativeCell(1)];*/

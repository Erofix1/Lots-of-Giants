...
#define CHIEFBUFFSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Chief Buff Banner Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Chief Buff Banner Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFBUFFSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefbuffsol_version", PLUGIN_VERSION, "[TF2] Be the Chief Buff Banner Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefbuffsol", Command_ChiefBuffBannerSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefBuffBannerSolSH);
...
{
//	CreateNative("BeChiefBuffBannerSol_MakeChiefBuffBannerSol", Native_SetChiefBuffBannerSol);
//	CreateNative("BeChiefBuffBannerSol_IsChiefBuffBannerSol", Native_IsChiefBuffBannerSol);
...
{
	if (g_bIsCHIEFBUFFSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFBUFFSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFBUFFSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFBUFFSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefBuffBannerSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeChiefBuffBannerSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Chief Buff Banner Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Chief Buff Banner Soldier !");
		PrintToChat(target_list[i], "2. Your Buff Banner starts with Full RAGE and lasts much longer than normal !");
		PrintToChat(target_list[i], "3. Chief has more health than Giant Buff Banner Soldier !");
...
{
	if (IsValidClient(client))
			GiveChiefBuffBannerSol(client);
}
 
stock GiveChiefBuffBannerSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFBUFFSOL[client] = true;
		...
		CreateWeapon(client, "tf_weapon_buff_item", 129, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 30014, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFBUFFSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChiefBuffBannerSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFBUFFSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefBuffBannerSol(Handle:plugin, args)
        MakeChiefBuffBannerSol(GetNativeCell(1));
 
public Native_IsChiefBuffBannerSol(Handle:plugin, args)
        return g_bIsCHIEFBUFFSOL[GetNativeCell(1)];*/

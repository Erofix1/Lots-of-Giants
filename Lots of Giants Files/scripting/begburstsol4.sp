...
#define GBURSTSOL4             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Burst Fire Soldier (Type 4)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Burst Fire Soldier (Type 4) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBURSTSOL4[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begburstsol4_version", PLUGIN_VERSION, "[TF2] Be the Giant Burst Fire Soldier (Type 4) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begburstsol4", Command_GiantBurstFireSol4, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBurstFireSol4SH);
...
{
//	CreateNative("BeGiantBurstFireSol4_MakeGiantBurstFireSol4", Native_SetGiantBurstFireSol4);
//	CreateNative("BeGiantBurstFireSol4_IsGiantBurstFireSol4", Native_IsGiantBurstFireSol4);
...
{
	if (g_bIsGBURSTSOL4[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBURSTSOL4[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBURSTSOL4);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBURSTSOL4[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBurstFireSol4(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBurstFireSol4(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Burst Fire Soldier (Type 4) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Burst Fire Soldier (Type 4) !");
		PrintToChat(target_list[i], "2. Your Rocket Launcher can fire a big burst of Rockets !");
		PrintToChat(target_list[i], "3. GateBot and deals 100 percent more damage than Type 1 !");
...
{
	if (IsValidClient(client))
			GiveGiantBurstFireSol4(client);
}
 
stock GiveGiantBurstFireSol4(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBURSTSOL4[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBURSTSOL4[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBurstFireSol4SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBURSTSOL4[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBurstFireSol4(Handle:plugin, args)
        MakeGiantBurstFireSol4(GetNativeCell(1));
 
public Native_IsGiantBurstFireSol4(Handle:plugin, args)
        return g_bIsGBURSTSOL4[GetNativeCell(1)];*/

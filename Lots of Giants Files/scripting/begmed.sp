...
public OnPluginStart()
{
	...
	CreateConVar("begmed_version", PLUGIN_VERSION, "[TF2] Be the Giant Medic version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begmed", Command_GiantMedic, ADMFLAG_ROOT, "It's a good time to run");
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGMEDIC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantMedic(client, args)
{
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Medic !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Medic !");
		PrintToChat(target_list[i], "2. Your Quick-Fix has insane Heal Rate.");
...
public Action:GiantMedicSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{

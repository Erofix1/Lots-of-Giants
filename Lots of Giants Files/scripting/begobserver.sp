...
#define GOBSERVER      "models/bots/spy/bot_spy.mdl"
#define SPAWN   "#mvm/giant_heavy/giant_heavy_entrance.wav"
#define DEATH   "mvm/sentrybuster/mvm_sentrybuster_explode.wav"

public Plugin:myinfo =
{
	name = "[TF2] Be the Giant Observer",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Observer from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGOBSERVER[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begobserver_version", PLUGIN_VERSION, "[TF2] Be the Giant Observer version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begobserver", Command_GiantObserver, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantObserverSH);
...
{
//	CreateNative("BeGiantObserver_MakeGiantObserver", Native_SetGiantObserver);
//	CreateNative("BeGiantObserver_IsGiantObserver", Native_IsGiantObserver);
...
{
	if (g_bIsGOBSERVER[client])
	{
		g_bIsGOBSERVER[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GOBSERVER);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
}
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGOBSERVER[client])
		{		
			TF2Attrib_RemoveAll(client);
...
public Action:Command_GiantObserver(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantObserver(target_list[i]);
...
MakeGiantObserver(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 875.0);
	...
	TF2Attrib_SetByName(client, "cloak consume rate decreased", 0.0000000001);
	TF2Attrib_SetByName(client, "cloak regen rate increased", 1000.0);
...
{
	if (IsValidClient(client))
		GiveGiantObserver(client);
}
 
stock GiveGiantObserver(client)
{
	if (IsValidClient(client))
	{
		g_bIsGOBSERVER[client] = true;
		...
		CreateWeapon(client, "tf_weapon_sapper", 810, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		TF2_RemoveWeaponSlot(client, 3);
		TF2_RemoveWeaponSlot(client, 4);
		CreateWeapon(client, "tf_weapon_invis", 60, 6, 1, 2, 0);
		
		CreateHat(client, 31048, 10, 6);
		CreateHat(client, 30848, 10, 6);
		CreateHat(client, 30009, 10, 6);

		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Building);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGOBSERVER[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Building);
...
public Action:GiantObserverSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGOBSERVER[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantObserver(Handle:plugin, args)
        MakeGiantObserver(GetNativeCell(1));
 
public Native_IsGiantObserver(Handle:plugin, args)
        return g_bIsGOBSERVER[GetNativeCell(1)];*/

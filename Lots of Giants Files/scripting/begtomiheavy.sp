...
#define GTOMIHEAVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Tomislav Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Tomislav Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGTOMIHEAVY[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begtomiheavy_version", PLUGIN_VERSION, "[TF2] Be the Giant Tomislav Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begtomiheavy", Command_GiantTomislavHeavy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantTomislavHeavySH);
...
{
//	CreateNative("BeGiantTomislavHeavy_MakeGiantTomislavHeavy", Native_SetGiantTomislavHeavy);
//	CreateNative("BeGiantTomislavHeavy_IsGiantTomislavHeavy", Native_IsGiantTomislavHeavy);
...
{
	if (g_bIsGTOMIHEAVY[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGTOMIHEAVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GTOMIHEAVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGTOMIHEAVY[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantTomislavHeavy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveGiantTomislavHeavy(client);
}
 
stock GiveGiantTomislavHeavy(client)
{
	if (IsValidClient(client))
	{
		g_bIsGTOMIHEAVY[client] = true;
		...
		CreateWeapon(client, "tf_weapon_minigun", 424, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGTOMIHEAVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantTomislavHeavySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGTOMIHEAVY[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantTomislavHeavy(Handle:plugin, args)
        MakeGiantTomislavHeavy(GetNativeCell(1));
 
public Native_IsGiantTomislavHeavy(Handle:plugin, args)
        return g_bIsGTOMIHEAVY[GetNativeCell(1)];*/

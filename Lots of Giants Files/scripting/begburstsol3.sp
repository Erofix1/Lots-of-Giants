...
#define GBURSTSOL3             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Burst Fire Soldier (Type 3)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Burst Fire Soldier (Type 3) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBURSTSOL3[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begburstsol3_version", PLUGIN_VERSION, "[TF2] Be the Giant Burst Fire Soldier (Type 3) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begburstsol3", Command_GiantBurstFireSol3, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBurstFireSol3SH);
...
{
//	CreateNative("BeGiantBurstFireSol3_MakeGiantBurstFireSol3", Native_SetGiantBurstFireSol3);
//	CreateNative("BeGiantBurstFireSol3_IsGiantBurstFireSol3", Native_IsGiantBurstFireSol3);
...
{
	if (g_bIsGBURSTSOL3[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBURSTSOL3[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBURSTSOL3);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBURSTSOL3[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBurstFireSol3(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBurstFireSol3(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Burst Fire Soldier (Type 3) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Burst Fire Soldier (Type 3) !");
		PrintToChat(target_list[i], "2. Your Rocket Launcher can fire a big burst of Rockets !");
		PrintToChat(target_list[i], "3. GateBot and deals 200 percent more damage than Type 1 !");
...
{
	if (IsValidClient(client))
			GiveGiantBurstFireSol3(client);
}
 
stock GiveGiantBurstFireSol3(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBURSTSOL3[client] = true;
		...
		CreateHat(client, 54, 10, 6);
		CreateHat(client, 339, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage bonus", 3.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBURSTSOL3[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBurstFireSol3SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBURSTSOL3[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBurstFireSol3(Handle:plugin, args)
        MakeGiantBurstFireSol3(GetNativeCell(1));
 
public Native_IsGiantBurstFireSol3(Handle:plugin, args)
        return g_bIsGBURSTSOL3[GetNativeCell(1)];*/

...
#define GBLACKBOXSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Black Box Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Black Box Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBLACKBOXSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begblackboxsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Black Box Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begblackboxsol", Command_GiantBlackBoxSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBlackBoxSolSH);
...
{
//	CreateNative("BeGiantBlackBoxSol_MakeGiantBlackBoxSol", Native_SetGiantBlackBoxSol);
//	CreateNative("BeGiantBlackBoxSol_IsGiantBlackBoxSol", Native_IsGiantBlackBoxSol);
...
{
	if (g_bIsGBLACKBOXSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBLACKBOXSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBLACKBOXSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBLACKBOXSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBlackBoxSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBlackBoxSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Black Box Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Black Box Soldier !");
		PrintToChat(target_list[i], "2. Your Black Box gives you lots of health when you deal damage with it.");
...
MakeGiantBlackBoxSol(client)
{
	...
	SetModel(client, GBLACKBOXSOL);
   
	TF2_SetHealth(client, 4200);
...
{
	if (IsValidClient(client))
			GiveGiantBlackBoxSol(client);
}
 
stock GiveGiantBlackBoxSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBLACKBOXSOL[client] = true;
		...
		CreateWeapon(client, "tf_weapon_rocketlauncher", 228, 6, 1, 2, 0);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "fire rate bonus", 0.001);
			TF2Attrib_SetByName(Weapon1, "reload time increased", 1.6);
			TF2Attrib_SetByName(Weapon1, "blast radius increased", 1.25);
			TF2Attrib_SetByName(Weapon1, "projectile spread angle penalty", 4.0);
			TF2Attrib_SetByName(Weapon1, "heal on hit for rapidfire", 1000.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBLACKBOXSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		...
		TF2Attrib_RemoveByName(Weapon1, "blast radius increased");
		TF2Attrib_RemoveByName(Weapon1, "projectile spread angle penalty");
		TF2Attrib_RemoveByName(Weapon1, "heal on hit for rapidfire");
...
public Action:GiantBlackBoxSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBLACKBOXSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBlackBoxSol(Handle:plugin, args)
        MakeGiantBlackBoxSol(GetNativeCell(1));
 
public Native_IsGiantBlackBoxSol(Handle:plugin, args)
        return g_bIsGBLACKBOXSOL[GetNativeCell(1)];*/

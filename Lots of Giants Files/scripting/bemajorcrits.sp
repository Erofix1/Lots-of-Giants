...
#define MAJORCRITS             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Major Crits",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Major Crits from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsMAJORCRITS[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bemajorcrits_version", PLUGIN_VERSION, "[TF2] Be the Major Crits version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bemajorcrits", Command_MajorCrits, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(MajorCritsSH);
...
{
//	CreateNative("BeMajorCrits_MakeMajorCrits", Native_SetMajorCrits);
//	CreateNative("BeMajorCrits_IsMajorCrits", Native_IsMajorCrits);
...
{
	if (g_bIsMAJORCRITS[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsMAJORCRITS[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(MAJORCRITS);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsMAJORCRITS[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_MajorCrits(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveMajorCrits(client);
}
 
stock GiveMajorCrits(client)
{
	if (IsValidClient(client))
	{
		g_bIsMAJORCRITS[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsMAJORCRITS[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:MajorCritsSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsMAJORCRITS[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetMajorCrits(Handle:plugin, args)
        MakeMajorCrits(GetNativeCell(1));
 
public Native_IsMajorCrits(Handle:plugin, args)
        return g_bIsMAJORCRITS[GetNativeCell(1)];*/

...
#define ARMOREDSANDMAN		"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Armored Sandman Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Armored Sandman Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsARMOREDSANDMAN[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bearmoredsandman_version", PLUGIN_VERSION, "[TF2] Be the Armored Sandman Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
...
{
//	CreateNative("BeArmoredSandmanScout_MakeArmoredSandmanScout", Native_SetArmoredSandmanScout);
//	CreateNative("BeArmoredSandmanScout_IsArmoredSandmanScout", Native_IsArmoredSandmanScout);
...
{
	if (g_bIsARMOREDSANDMAN[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsARMOREDSANDMAN[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(ARMOREDSANDMAN);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsARMOREDSANDMAN[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ArmoredSandmanScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeArmoredSandmanScout(client)
{
	...
	SetModel(client, ARMOREDSANDMAN);
   
	TF2_SetHealth(client, 3000);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", true);
	TF2Attrib_SetByName(client, "max health additive bonus", 2890.0);
	...
	g_bIsARMOREDSANDMAN[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveArmoredSandmanScout(client);
}
 
stock GiveArmoredSandmanScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsARMOREDSANDMAN[client] = true;
		...
		CreateHat(client, 52, 10, 6);
		CreateHat(client, 30770, 10, 6);
		CreateHat(client, 30771, 10, 6);
		...
		if(IsValidEntity(Weapon3))
		{
			...
			TF2Attrib_SetByName(Weapon3, "effect bar recharge rate increased", 0.05);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsARMOREDSANDMAN[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:ArmoredSandmanScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsARMOREDSANDMAN[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetArmoredSandmanScout(Handle:plugin, args)
        MakeArmoredSandmanScout(GetNativeCell(1));
 
public Native_IsArmoredSandmanScout(Handle:plugin, args)
        return g_bIsARMOREDSANDMAN[GetNativeCell(1)];*/

...
#define SUPERSCOUT		"models/bots/scout_boss/bot_scout_boss.mdl"
...
new bool:g_bIsSUPERSCOUT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("besupers_version", PLUGIN_VERSION, "[TF2] Be the Super Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_besupers", Command_SuperScout, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(SuperScoutSH);
...
{
//	CreateNative("BeSuperScout_MakeSuperScout", Native_SetSuperScout);
//	CreateNative("BeSuperScout_IsSuperScout", Native_IsSuperScout);
...
{
	if (g_bIsSUPERSCOUT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsSUPERSCOUT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(SUPERSCOUT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSUPERSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SuperScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveSuperScout(client);
}
 
stock GiveSuperScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsSUPERSCOUT[client] = true;
		...
		CreateWeapon(client, "tf_weapon_bat_fish", 221, 6, 1, 2, 0);

		CreateHat(client, 451, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSUPERSCOUT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:SuperScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsSUPERSCOUT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetSuperScout(Handle:plugin, args)
        MakeSuperScout(GetNativeCell(1));
 
public Native_IsSuperScout(Handle:plugin, args)
        return g_bIsSUPERSCOUT[GetNativeCell(1)];*/

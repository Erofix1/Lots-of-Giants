...
#define GBLASTSOL2             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Unused Giant Blast Soldier (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Blast Soldier (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBLASTSOL2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begblastsol2_version", PLUGIN_VERSION, "[TF2] Be the Unused Giant Blast Soldier (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begblastsol2", Command_GiantBlastSol2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBlastSol2SH);
...
{
//	CreateNative("BeGiantBlastSol2_MakeGiantBlastSol2", Native_SetGiantBlastSol2);
//	CreateNative("BeGiantBlastSol2_IsGiantBlastSol2", Native_IsGiantBlastSol2);
...
{
	if (g_bIsGBLASTSOL2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBLASTSOL2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBLASTSOL2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBLASTSOL2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBlastSol2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBlastSol2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Blast Soldier (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Blast Soldier (Type 2) !");
		PrintToChat(target_list[i], "2. Your Rockets launch enemies into the air !");
		PrintToChat(target_list[i], "3. Type 2 is a GateBot, has more health and gives less RAGE.");
...
MakeGiantBlastSol2(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 3800.0);
...
{
	if (IsValidClient(client))
			GiveGiantBlastSol2(client);
}
 
stock GiveGiantBlastSol2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBLASTSOL2[client] = true;
		...
		CreateWeapon(client, "tf_weapon_rocketlauncher", 414, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 252, 10, 6);
		CreateHat(client, 1063, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBLASTSOL2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBlastSol2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBLASTSOL2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBlastSol2(Handle:plugin, args)
        MakeGiantBlastSol2(GetNativeCell(1));
 
public Native_IsGiantBlastSol2(Handle:plugin, args)
        return g_bIsGBLASTSOL2[GetNativeCell(1)];*/

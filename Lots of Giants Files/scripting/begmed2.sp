...
#define GMEDIC2             "models/bots/medic/bot_medic.mdl"
...
{
	name = "[TF2] Be the Giant Medic (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Medic (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGMEDIC2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begmed2_version", PLUGIN_VERSION, "[TF2] Be the Giant Medic (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begmed2", Command_GiantMedic2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantMedic2SH);
...
{
//	CreateNative("BeGiantMedic2_MakeGiantMedic2", Native_SetGiantMedic2);
//	CreateNative("BeGiantMedic2_IsGiantMedic2", Native_IsGiantMedic2);
...
{
	if (g_bIsGMEDIC2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGMEDIC2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GMEDIC2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGMEDIC2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantMedic2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantMedic2(client)
{
	...
	SetModel(client, GMEDIC2);
  
	TF2_SetHealth(client, 4500);
...
{
	if (IsValidClient(client))
			GiveGiantMedic2(client);
}
 
stock GiveGiantMedic2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGMEDIC2[client] = true;
		...
		CreateWeapon(client, "tf_weapon_syringegun_medic", 17, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		CreateWeapon(client, "tf_weapon_medigun", 411, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 616, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage penalty", 0.1);
		...
		if(IsValidEntity(Weapon2))
		{
			...
			TF2Attrib_SetByName(Weapon2, "heal rate bonus", 200.0);
			TF2Attrib_SetByName(Weapon2, "overheal penalty", 1.0);
			TF2Attrib_SetByName(Weapon2, "ubercharge rate penalty", 0.01);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGMEDIC2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
	...
	}
	{
		...
		TF2Attrib_RemoveByName(Weapon2, "overheal penalty");
		TF2Attrib_RemoveByName(Weapon2, "ubercharge rate penalty");
		TF2Attrib_RemoveByName(Weapon2, "killstreak tier");	
	}
}
 
public Action:GiantMedic2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGMEDIC2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantMedic2(Handle:plugin, args)
        MakeGiantMedic2(GetNativeCell(1));
 
public Native_IsGiantMedic2(Handle:plugin, args)
        return g_bIsGMEDIC2[GetNativeCell(1)];*/

...
#define CHIEFRAPIDDEMO		"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Unused Giant Rapid Fire Demoman Chief",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Rapid Fire Demoman Chief from MvM",
...
new bool:g_bIsCHIEFRAPIDDEMO[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefrapidde_version", PLUGIN_VERSION, "[TF2] Be the Unused Giant Rapid Fire Demoman Chief version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefrapidde", Command_ChiefRapidDemo, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefRapidDemoSH);
...
{
//	CreateNative("BeChiefRapidDemo_MakeChiefRapidDemo", Native_SetChiefRapidDemo);
//	CreateNative("BeChiefRapidDemo_IsChiefRapidDemo", Native_IsChiefRapidDemo);
...
{
	if (g_bIsCHIEFRAPIDDEMO[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFRAPIDDEMO[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFRAPIDDEMO);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFRAPIDDEMO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefRapidDemo(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeChiefRapidDemo(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Rapid Fire Demoman Chief !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Rapid Fire Demoman Chief !");
		PrintToChat(target_list[i], "2. Your Grenade Launcher fires a burst of crit boosted Pipes.");
...
MakeChiefRapidDemo(client)
{
	...
	SetModel(client, CHIEFRAPIDDEMO);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 59825.0);
	...
	g_bIsCHIEFRAPIDDEMO[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveChiefRapidDemo(client);
}
 
stock GiveChiefRapidDemo(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFRAPIDDEMO[client] = true;
		...
		CreateWeapon(client, "tf_weapon_grenadelauncher", 19, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		TF2_RemoveWeaponSlot(client, 2);

		CreateHat(client, 390, 10, 6);
		CreateHat(client, 295, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "faster reload rate", 0.65);
			TF2Attrib_SetByName(Weapon1, "projectile speed increased", 1.1);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFRAPIDDEMO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		...
		TF2Attrib_RemoveByName(Weapon1, "projectile speed increased");
...
public Action:ChiefRapidDemoSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFRAPIDDEMO[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefRapidDemo(Handle:plugin, args)
        MakeChiefRapidDemo(GetNativeCell(1));
 
public Native_IsChiefRapidDemo(Handle:plugin, args)
        return g_bIsCHIEFRAPIDDEMO[GetNativeCell(1)];*/

...
#define GBUFFSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Buff Banner Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Buff Banner Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBUFFSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begbuffsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Buff Banner Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begbuffsol", Command_GiantBuffBannerSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBuffBannerSolSH);
...
{
//	CreateNative("BeGiantBuffBannerSol_MakeGiantBuffBannerSol", Native_SetGiantBuffBannerSol);
//	CreateNative("BeGiantBuffBannerSol_IsGiantBuffBannerSol", Native_IsGiantBuffBannerSol);
...
{
	if (g_bIsGBUFFSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBUFFSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBUFFSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBUFFSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBuffBannerSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBuffBannerSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Buff Banner Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Buff Banner Soldier !");
...
{
	if (IsValidClient(client))
			GiveGiantBuffBannerSol(client);
}
 
stock GiveGiantBuffBannerSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBUFFSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBUFFSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBuffBannerSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBUFFSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBuffBannerSol(Handle:plugin, args)
        MakeGiantBuffBannerSol(GetNativeCell(1));
 
public Native_IsGiantBuffBannerSol(Handle:plugin, args)
        return g_bIsGBUFFSOL[GetNativeCell(1)];*/

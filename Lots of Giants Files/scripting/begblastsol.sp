...
#define GBLASTSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Blast Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Blast Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBLASTSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begblastsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Blast Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begblastsol", Command_GiantBlastSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBlastSolSH);
...
{
//	CreateNative("BeGiantBlastSol_MakeGiantBlastSol", Native_SetGiantBlastSol);
//	CreateNative("BeGiantBlastSol_IsGiantBlastSol", Native_IsGiantBlastSol);
...
{
	if (g_bIsGBLASTSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBLASTSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBLASTSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBLASTSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBlastSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBlastSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Blast Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Blast Soldier !");
...
{
	if (IsValidClient(client))
			GiveGiantBlastSol(client);
}
 
stock GiveGiantBlastSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBLASTSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBLASTSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBlastSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBLASTSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBlastSol(Handle:plugin, args)
        MakeGiantBlastSol(GetNativeCell(1));
 
public Native_IsGiantBlastSol(Handle:plugin, args)
        return g_bIsGBLASTSOL[GetNativeCell(1)];*/

...
#define RAPIDBOWMAN		"models/bots/sniper/bot_sniper.mdl"
...
{
	name = "[TF2] Be the Rapid Fire Bowman",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Rapid Fire Bowman from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsRAPIDBOWMAN[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("berapidbowman_version", PLUGIN_VERSION, "[TF2] Be the Rapid Fire Bowman version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_berapidbowman", Command_RapidFireBowman, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(RapidFireBowmanSH);
...
{
//	CreateNative("BeRapidFireBowman_MakeRapidFireBowman", Native_SetRapidFireBowman);
//	CreateNative("BeRapidFireBowman_IsRapidFireBowman", Native_IsRapidFireBowman);
...
{
	if (g_bIsRAPIDBOWMAN[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsRAPIDBOWMAN[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(RAPIDBOWMAN);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsRAPIDBOWMAN[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_RapidFireBowman(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeRapidFireBowman(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Rapid Fire Bowman !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Rapid Fire Bowman !");
		PrintToChat(target_list[i], "2. Your Huntsman is crit boosted and charges up the arrow much faster !");
...
{
	if (IsValidClient(client))
			GiveRapidFireBowman(client);
}
 
stock GiveRapidFireBowman(client)
{
	if (IsValidClient(client))
	{
		g_bIsRAPIDBOWMAN[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsRAPIDBOWMAN[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:RapidFireBowmanSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsRAPIDBOWMAN[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetRapidFireBowman(Handle:plugin, args)
        MakeRapidFireBowman(GetNativeCell(1));
 
public Native_IsRapidFireBowman(Handle:plugin, args)
        return g_bIsRAPIDBOWMAN[GetNativeCell(1)];*/

...
#define FANSUPERSCOUT2	"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Force-a-Nature Super Scout (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Force-a-Nature Super Scout (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsFANSUPERSCOUT2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("befansupers2_version", PLUGIN_VERSION, "[TF2] Be the Force-a-Nature Super Scout (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_befansupers2", Command_FaNSuperScout2, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(FaNSuperScout2SH);
...
{
//	CreateNative("BeFaNSuperScout2_MakeFaNSuperScout2", Native_SetFaNSuperScout2);
//	CreateNative("BeFaNSuperScout2_IsFaNSuperScout2", Native_IsFaNSuperScout2);
...
{
	if (g_bIsFANSUPERSCOUT2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsFANSUPERSCOUT2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(FANSUPERSCOUT2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsFANSUPERSCOUT2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_FaNSuperScout2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeFaNSuperScout2(client)
{
	...
	SetModel(client, FANSUPERSCOUT2);
	...
	g_bIsFANSUPERSCOUT2[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveFaNSuperScout2(client);
}
 
stock GiveFaNSuperScout2(client)
{
	if (IsValidClient(client))
	{
		g_bIsFANSUPERSCOUT2[client] = true;
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "scattergun knockback mult", 3.5);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsFANSUPERSCOUT2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:FaNSuperScout2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsFANSUPERSCOUT2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetFaNSuperScout2(Handle:plugin, args)
        MakeFaNSuperScout2(GetNativeCell(1));
 
public Native_IsFaNSuperScout2(Handle:plugin, args)
        return g_bIsFANSUPERSCOUT2[GetNativeCell(1)];*/

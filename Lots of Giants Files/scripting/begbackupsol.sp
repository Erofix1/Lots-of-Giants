...
#define GBACKUPSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Battalion's Backup Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Battalion's Backup Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBACKUPSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begbackupsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Battalion's Backup Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begbackupsol", Command_GiantBattalionsBackupSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBattalionsBackupSolSH);
...
{
//	CreateNative("BeGiantBattalionsBackupSol_MakeGiantBattalionsBackupSol", Native_SetGiantBattalionsBackupSol);
//	CreateNative("BeGiantBattalionsBackupSol_IsGiantBattalionsBackupSol", Native_IsGiantBattalionsBackupSol);
...
{
	if (g_bIsGBACKUPSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBACKUPSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBACKUPSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBACKUPSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBattalionsBackupSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBattalionsBackupSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Battalion's Backup Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Battalion's Backup Soldier !");
...
MakeGiantBattalionsBackupSol(client)
{
...
{
	if (IsValidClient(client))
			GiveGiantBattalionsBackupSol(client);
}
 
stock GiveGiantBattalionsBackupSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBACKUPSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBACKUPSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBattalionsBackupSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBACKUPSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBattalionsBackupSol(Handle:plugin, args)
        MakeGiantBattalionsBackupSol(GetNativeCell(1));
 
public Native_IsGiantBattalionsBackupSol(Handle:plugin, args)
        return g_bIsGBACKUPSOL[GetNativeCell(1)];*/

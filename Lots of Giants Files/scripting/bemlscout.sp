...
#define MLSCOUT		"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Major League Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Major League Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsMLSCOUT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bemlscout_version", PLUGIN_VERSION, "[TF2] Be the Major League Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bemlscout", Command_MajorLeagueScout, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(MajorLeagueScoutSH);
...
{
//	CreateNative("BeMajorLeagueScout_MakeMajorLeagueScout", Native_SetMajorLeagueScout);
//	CreateNative("BeMajorLeagueScout_IsMajorLeagueScout", Native_IsMajorLeagueScout);
...
{
	if (g_bIsMLSCOUT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsMLSCOUT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(MLSCOUT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsMLSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_MajorLeagueScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeMajorLeagueScout(client)
{
	...
	SetModel(client, MLSCOUT);
   
	TF2_SetHealth(client, 1600);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", true);
	TF2Attrib_SetByName(client, "max health additive bonus", 1490.0);
...
{
	if (IsValidClient(client))
			GiveMajorLeagueScout(client);
}
 
stock GiveMajorLeagueScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsMLSCOUT[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsMLSCOUT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:MajorLeagueScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsMLSCOUT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetMajorLeagueScout(Handle:plugin, args)
        MakeMajorLeagueScout(GetNativeCell(1));
 
public Native_IsMajorLeagueScout(Handle:plugin, args)
        return g_bIsMLSCOUT[GetNativeCell(1)];*/

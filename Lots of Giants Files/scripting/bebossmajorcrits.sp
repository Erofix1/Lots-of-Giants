...
#define BMAJORCRITS             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Boss Major Crits",
	author = "Erofix but these guys are the Real MVPs ----> Pelipoika, PC Gamer, Jaster and StormishJustice)",
	description = "Play as the Boss Major Crits from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsBMAJORCRITS[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bebossmajorcrits_version", PLUGIN_VERSION, "[TF2] Be the Boss Major Crits version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bebossmajorcrits", Command_BossMajorCrits, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(BossMajorCritsSH);
...
{
//	CreateNative("BeBossMajorCrits_MakeBossMajorCrits", Native_SetBossMajorCrits);
//	CreateNative("BeBossMajorCrits_IsBossMajorCrits", Native_IsBossMajorCrits);
...
{
	if (g_bIsBMAJORCRITS[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsBMAJORCRITS[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(BMAJORCRITS);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsBMAJORCRITS[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_BossMajorCrits(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeBossMajorCrits(client)
{
	...
	SetModel(client, BMAJORCRITS);
	
	TF2_SetHealth(client, 32000);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 31800.0);
...
{
	if (IsValidClient(client))
			GiveBossMajorCrits(client);
}
 
stock GiveBossMajorCrits(client)
{
	if (IsValidClient(client))
	{
		g_bIsBMAJORCRITS[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsBMAJORCRITS[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:BossMajorCritsSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsBMAJORCRITS[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetBossMajorCrits(Handle:plugin, args)
        MakeBossMajorCrits(GetNativeCell(1));
 
public Native_IsBossMajorCrits(Handle:plugin, args)
        return g_bIsBMAJORCRITS[GetNativeCell(1)];*/

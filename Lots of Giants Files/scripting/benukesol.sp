...
#define MAJORCRITSATOMIC    "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Major Crits Atomic",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Major Crits Atomic from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsMAJORCRITSATOMIC[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("benukesol_version", PLUGIN_VERSION, "[TF2] Be the Major Crits Atomic version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_benukesol", Command_MajorCritsAtomic, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(MajorCritsAtomicSH);
...
{
//	CreateNative("BeMajorCritsAtomic_MakeMajorCritsAtomic", Native_SetMajorCritsAtomic);
//	CreateNative("BeMajorCritsAtomic_IsMajorCritsAtomic", Native_IsMajorCritsAtomic);
...
{
	if (g_bIsMAJORCRITSATOMIC[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsMAJORCRITSATOMIC[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(MAJORCRITSATOMIC);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsMAJORCRITSATOMIC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_MajorCritsAtomic(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeMajorCritsAtomic(client)
{
	...
	SetModel(client, MAJORCRITSATOMIC);
   
	TF2_SetHealth(client, 40000);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 39800.0);
	...
	g_bIsMAJORCRITSATOMIC[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveMajorCritsAtomic(client);
}
 
stock GiveMajorCritsAtomic(client)
{
	if (IsValidClient(client))
	{
		g_bIsMAJORCRITSATOMIC[client] = true;
		...
		CreateWeapon(client, "tf_wearable", 133, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);

		CreateHat(client, 378, 10, 6);
		CreateHat(client, 446, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "reload time increased", 3.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsMAJORCRITSATOMIC[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:MajorCritsAtomicSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsMAJORCRITSATOMIC[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetMajorCritsAtomic(Handle:plugin, args)
        MakeMajorCritsAtomic(GetNativeCell(1));
 
public Native_IsMajorCritsAtomic(Handle:plugin, args)
        return g_bIsMAJORCRITSATOMIC[GetNativeCell(1)];*/

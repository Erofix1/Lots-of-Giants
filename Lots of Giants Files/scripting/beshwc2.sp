...
#define SHWC2             "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Super Heavyweight Champ (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Super Heavyweight Champ (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsSHWC2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beshwc2_version", PLUGIN_VERSION, "[TF2] Be the Super Heavyweight Champ (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beshwc2", Command_SuperHeavyweightChamp2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(SuperHeavyweightChamp2SH);
...
{
//	CreateNative("BeSuperHeavyweightChamp2_MakeSuperHeavyweightChamp2", Native_SetSuperHeavyweightChamp2);
//	CreateNative("BeSuperHeavyweightChamp2_IsSuperHeavyweightChamp2", Native_IsSuperHeavyweightChamp2);
...
{
	if (g_bIsSHWC2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsSHWC2[client] = false;
	}
}
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSHWC2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SuperHeavyweightChamp2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeSuperHeavyweightChamp2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Super Heavyweight Champ (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Super Heavyweight Champ (Type 2) !");
		PrintToChat(target_list[i], "2. Type 2 uses the pre-Jungle Inferno G.R.U. .");
...
MakeSuperHeavyweightChamp2(client)
{
...
{
	if (IsValidClient(client))
			GiveSuperHeavyweightChamp2(client);
}
 
stock GiveSuperHeavyweightChamp2(client)
{
	if (IsValidClient(client))
	{
		g_bIsSHWC2[client] = true;
		...
		CreateWeapon(client, "tf_weapon_fists", 1184, 6, 1, 2, 0);
		...
		if(IsValidEntity(Weapon3))
		{
			...
			TF2Attrib_SetByName(Weapon3, "fire rate bonus", 0.6);
			TF2Attrib_SetByName(Weapon3, "damage bonus", 1.2);
			TF2Attrib_SetByName(Weapon3, "damage penalty", 1.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSHWC2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
		
		TF2Attrib_RemoveByName(Weapon3, "fire rate bonus");
		TF2Attrib_RemoveByName(Weapon3, "damage penalty");
		TF2Attrib_RemoveByName(Weapon3, "damage bonus");
		TF2Attrib_RemoveByName(Weapon3, "killstreak tier");
	}
}
 
public Action:SuperHeavyweightChamp2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsSHWC2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetSuperHeavyweightChamp2(Handle:plugin, args)
        MakeSuperHeavyweightChamp2(GetNativeCell(1));
 
public Native_IsSuperHeavyweightChamp2(Handle:plugin, args)
        return g_bIsSHWC2[GetNativeCell(1)];*/

...
#define GBLACKBOXSOL2             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Black Box Soldier (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Black Box Soldier (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBLACKBOXSOL2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begblackboxsol2_version", PLUGIN_VERSION, "[TF2] Be the Giant Black Box Soldier (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begblackboxsol2", Command_GiantBlackBoxSol2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBlackBoxSol2SH);
...
{
//	CreateNative("BeGiantBlackBoxSol2_MakeGiantBlackBoxSol2", Native_SetGiantBlackBoxSol2);
//	CreateNative("BeGiantBlackBoxSol2_IsGiantBlackBoxSol2", Native_IsGiantBlackBoxSol2);
...
{
	if (g_bIsGBLACKBOXSOL2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBLACKBOXSOL2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBLACKBOXSOL2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBLACKBOXSOL2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBlackBoxSol2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantBlackBoxSol2(client)
{
	...
	g_bIsGBLACKBOXSOL2[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveGiantBlackBoxSol2(client);
}
 
stock GiveGiantBlackBoxSol2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBLACKBOXSOL2[client] = true;
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage penalty", 0.33);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBLACKBOXSOL2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBlackBoxSol2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBLACKBOXSOL2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBlackBoxSol2(Handle:plugin, args)
        MakeGiantBlackBoxSol2(GetNativeCell(1));
 
public Native_IsGiantBlackBoxSol2(Handle:plugin, args)
        return g_bIsGBLACKBOXSOL2[GetNativeCell(1)];*/

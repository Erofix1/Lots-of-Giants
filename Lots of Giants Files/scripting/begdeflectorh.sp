...
public OnPluginStart()
{
	...
	CreateConVar("begdeflectorh_version", PLUGIN_VERSION, "[TF2] Be the Giant Deflector Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begdeflectorh", Command_GDeflectorH, ADMFLAG_ROOT, "It's a good time to run");
...
public Action:Command_GDeflectorH(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
public Action:GDeflectorHSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{

...
#define CHIEFPYRO		"models/bots/pyro_boss/bot_pyro_boss.mdl"
...
public Plugin:myinfo = 
{
	...
	description = "Play as the Unused Chief Pyro from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFPYRO[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("bechiefpyro_version", PLUGIN_VERSION, "[TF2] Be the Chief Pyro version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
	
	RegAdminCmd("sm_bechiefpyro", Command_ChiefPyro, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(ChiefPyroSH);
...
{
//	CreateNative("BeChiefPyro_MakeChiefPyro", Native_SetChiefPyro);
//	CreateNative("BeChiefPyro_IsChiefPyro", Native_IsChiefPyro);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFPYRO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefPyro(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveChiefPyro(client);
}

stock GiveChiefPyro(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFPYRO[client] = true;
		...
		CreateHat(client, 316, 10, 6);
		CreateHat(client, 30168, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "airblast pushback scale", 2.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFPYRO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChiefPyroSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFPYRO[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsCHIEFPYRO[iClient]) 
	{	
...
public Native_SetChiefPyro(Handle:plugin, args)
	MakeChiefPyro(GetNativeCell(1));

public Native_IsChiefPyro(Handle:plugin, args)
	return g_bIsCHIEFPYRO[GetNativeCell(1)];

...
#define CAPTAINPUNCH       "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
public Plugin:myinfo =
{
	...
	description = "Play as the Captain Punch from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsCAPTAINPUNCH[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("becapp_version", PLUGIN_VERSION, "[TF2] Be the Captain Punch version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);

	RegAdminCmd("sm_becapp", Command_CaptainPunch, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(CaptainPunchSH);
...
{
//	CreateNative("BeCaptainPunch_MakeCaptainPunch", Native_SetCaptainPunch);
//	CreateNative("BeCaptainPunch_IsCaptainPunch", Native_IsCaptainPunch);
...
{
	if (g_bIsCAPTAINPUNCH[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCAPTAINPUNCH[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CAPTAINPUNCH);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCAPTAINPUNCH[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_CaptainPunch(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveCaptainPunch(client);
}
 
stock GiveCaptainPunch(client)
{
	if (IsValidClient(client))
	{
		g_bIsCAPTAINPUNCH[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCAPTAINPUNCH[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:CaptainPunchSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCAPTAINPUNCH[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetCaptainPunch(Handle:plugin, args)
        MakeCaptainPunch(GetNativeCell(1));
 
public Native_IsCaptainPunch(Handle:plugin, args)
        return g_bIsCAPTAINPUNCH[GetNativeCell(1)];*/

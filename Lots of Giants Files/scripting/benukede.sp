...
#define SIRNUKESALOT		"models/bots/demo_boss/bot_demo_boss.mdl"
...
new bool:g_bIsSIRNUKESALOT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("benukede_version", PLUGIN_VERSION, "[TF2] Be the Sir Nukesalot version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_benukede", Command_SirNukesalot, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(SirNukesalotSH);
...
{
//	CreateNative("BeSirNukesalot_MakeSirNukesalot", Native_SetSirNukesalot);
//	CreateNative("BeSirNukesalot_IsSirNukesalot", Native_IsSirNukesalot);
...
{
	if (g_bIsSIRNUKESALOT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsSIRNUKESALOT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(SIRNUKESALOT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSIRNUKESALOT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SirNukesalot(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeSirNukesalot(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Sir Nukesalot !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Sir Nukesalot !");
		PrintToChat(target_list[i], "2. Your Loose Cannon fires crit boosted bombs of pure Death.");
...
MakeSirNukesalot(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 49825.0);
	TF2Attrib_SetByName(client, "ammo regen", 100.0);
	TF2Attrib_SetByName(client, "damage force reduction", 0.4);
	TF2Attrib_SetByName(client, "move speed penalty", 0.35);
...
{
	if (IsValidClient(client))
			GiveSirNukesalot(client);
}
 
stock GiveSirNukesalot(client)
{
	if (IsValidClient(client))
	{
		g_bIsSIRNUKESALOT[client] = true;
		...
		CreateWeapon(client, "tf_weapon_cannon", 996, 6, 1, 2, 0);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage bonus", 7.0);
			...
			TF2Attrib_SetByName(Weapon1, "blast radius increased", 1.2);
			...
			TF2Attrib_SetByName(Weapon1, "reload time increased", 1.8);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSIRNUKESALOT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		TF2Attrib_RemoveByName(Weapon1, "damage bonus");
		TF2Attrib_RemoveByName(Weapon1, "projectile spread angle penalty");
		TF2Attrib_RemoveByName(Weapon1, "grenade launcher mortar mode");
		TF2Attrib_RemoveByName(Weapon1, "damage causes airblast");
		TF2Attrib_RemoveByName(Weapon1, "blast radius increased");
		TF2Attrib_RemoveByName(Weapon1, "use large smoke explosion");
		TF2Attrib_RemoveByName(Weapon1, "fire rate penalty");
		TF2Attrib_RemoveByName(Weapon1, "clip size penalty");
...
public Action:SirNukesalotSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsSIRNUKESALOT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetSirNukesalot(Handle:plugin, args)
        MakeSirNukesalot(GetNativeCell(1));
 
public Native_IsSirNukesalot(Handle:plugin, args)
        return g_bIsSIRNUKESALOT[GetNativeCell(1)];*/

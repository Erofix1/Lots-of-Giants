...
#define FANSUPERSCOUT	"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Force-a-Nature Super Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Force-a-Nature Super Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsFANSUPERSCOUT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("befansupers_version", PLUGIN_VERSION, "[TF2] Be the Force-a-Nature Super Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_befansupers", Command_FaNSuperScout, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(FaNSuperScoutSH);
...
{
//	CreateNative("BeFaNSuperScout_MakeFaNSuperScout", Native_SetFaNSuperScout);
//	CreateNative("BeFaNSuperScout_IsFaNSuperScout", Native_IsFaNSuperScout);
...
{
	if (g_bIsFANSUPERSCOUT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsFANSUPERSCOUT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(FANSUPERSCOUT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsFANSUPERSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_FaNSuperScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeFaNSuperScout(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Force-a-Nature Super Scout !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Force-a-Nature Super Scout !");
		PrintToChat(target_list[i], "2. Your Force-a-Nature has much stronger knockback effect but reloads slower and deals less damage.");
...
MakeFaNSuperScout(client)
{
	...
	SetModel(client, FANSUPERSCOUT);
   
	TF2_SetHealth(client, 1200);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", true);
	TF2Attrib_SetByName(client, "max health additive bonus", 1075.0);
	...
	g_bIsFANSUPERSCOUT[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveFaNSuperScout(client);
}
 
stock GiveFaNSuperScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsFANSUPERSCOUT[client] = true;
		...
		CreateWeapon(client, "tf_weapon_scattergun", 45, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 780, 10, 6);
		CreateHat(client, 30027, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "reload time increased", 1.7);
			TF2Attrib_SetByName(Weapon1, "scattergun knockback mult", 6.0);
			TF2Attrib_SetByName(Weapon1, "damage penalty", 0.35);
			TF2Attrib_SetByName(Weapon1, "weapon spread bonus", 0.4);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsFANSUPERSCOUT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		TF2Attrib_RemoveByName(Weapon1, "bullets per shot bonus");
		TF2Attrib_RemoveByName(Weapon1, "fire rate bonus");
		TF2Attrib_RemoveByName(Weapon1, "reload time increased");
		TF2Attrib_RemoveByName(Weapon1, "scattergun knockback mult");
		TF2Attrib_RemoveByName(Weapon1, "damage penalty");
		TF2Attrib_RemoveByName(Weapon1, "weapon spread bonus");
		TF2Attrib_RemoveByName(Weapon1, "maxammo primary increased");
		TF2Attrib_RemoveByName(Weapon1, "killstreak tier");
	}
}
 
public Action:FaNSuperScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsFANSUPERSCOUT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetFaNSuperScout(Handle:plugin, args)
        MakeFaNSuperScout(GetNativeCell(1));
 
public Native_IsFaNSuperScout(Handle:plugin, args)
        return g_bIsFANSUPERSCOUT[GetNativeCell(1)];*/

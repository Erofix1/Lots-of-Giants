...
#define CHARGEDSOLDIER       "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Charged Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Charged Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHARGEDSOLDIER[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechargedsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Charged Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechargedsol", Command_ChargedSoldier, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChargedSoldierSH);
...
{
//	CreateNative("BeChargedSoldier_MakeChargedSoldier", Native_SetChargedSoldier);
//	CreateNative("BeChargedSoldier_IsChargedSoldier", Native_IsChargedSoldier);
...
{
	if (g_bIsCHARGEDSOLDIER[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHARGEDSOLDIER[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHARGEDSOLDIER);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHARGEDSOLDIER[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChargedSoldier(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeChargedSoldier(client)
{
	...
	SetModel(client, CHARGEDSOLDIER);
	...
	g_bIsCHARGEDSOLDIER[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveChargedSoldier(client);
}
 
stock GiveChargedSoldier(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHARGEDSOLDIER[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHARGEDSOLDIER[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChargedSoldierSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHARGEDSOLDIER[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChargedSoldier(Handle:plugin, args)
        MakeChargedSoldier(GetNativeCell(1));
 
public Native_IsChargedSoldier(Handle:plugin, args)
        return g_bIsCHARGEDSOLDIER[GetNativeCell(1)];*/

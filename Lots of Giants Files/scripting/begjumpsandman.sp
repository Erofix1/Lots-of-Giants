...
#define GJUMPSANDMAN		"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Giant Jumping Sandman Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Jumping Sandman Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGJUMPSANDMAN[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begjumpsandman_version", PLUGIN_VERSION, "[TF2] Be the Giant Jumping Sandman Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begjumpsandman", Command_GiantJumpingSandman, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(GiantJumpingSandmanSH);
...
{
//	CreateNative("BeGiantJumpingSandman_MakeGiantJumpingSandman", Native_SetGiantJumpingSandman);
//	CreateNative("BeGiantJumpingSandman_IsGiantJumpingSandman", Native_IsGiantJumpingSandman);
...
{
	if (g_bIsGJUMPSANDMAN[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGJUMPSANDMAN[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GJUMPSANDMAN);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGJUMPSANDMAN[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantJumpingSandman(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantJumpingSandman(client)
{
	...
	SetModel(client, GJUMPSANDMAN);
   
	TF2_SetHealth(client, 1200);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", true);
	TF2Attrib_SetByName(client, "max health additive bonus", 1090.0);
	...
	TF2Attrib_SetByName(client, "head scale", 1.5);
...
{
	if (IsValidClient(client))
			GiveGiantJumpingSandman(client);
}
 
stock GiveGiantJumpingSandman(client)
{
	if (IsValidClient(client))
	{
		g_bIsGJUMPSANDMAN[client] = true;
		...
		CreateHat(client, 858, 10, 6);
		CreateHat(client, 859, 10, 6);
		...
		if(IsValidEntity(Weapon3))
		{
			...
			TF2Attrib_SetByName(Weapon3, "effect bar recharge rate increased", 0.1);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGJUMPSANDMAN[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:GiantJumpingSandmanSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGJUMPSANDMAN[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantJumpingSandman(Handle:plugin, args)
        MakeGiantJumpingSandman(GetNativeCell(1));
 
public Native_IsGiantJumpingSandman(Handle:plugin, args)
        return g_bIsGJUMPSANDMAN[GetNativeCell(1)];*/

...
#define GBBHEAVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Brass Beast Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Brass Beast Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGBBHEAVY[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("begbbheavy_version", PLUGIN_VERSION, "[TF2] Be the Giant Brass Beast Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begbbheavy", Command_GBrassBeastHeavy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GBrassBeastHeavySH);
...
{
//	CreateNative("BeGBrassBeastHeavy_MakeGBrassBeastHeavy", Native_SetGBrassBeastHeavy);
//	CreateNative("BeGBrassBeastHeavy_IsGBrassBeastHeavy", Native_IsGBrassBeastHeavy);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGBBHEAVY[client])
	{
		...
		g_bIsGBBHEAVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBBHEAVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBBHEAVY[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_GBrassBeastHeavy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveGBrassBeastHeavy(client);
}
 
stock GiveGBrassBeastHeavy(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBBHEAVY[client] = true;
		...
		CreateWeapon(client, "tf_weapon_minigun", 312, 6, 1, 2, 0);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage bonus", 1.7);
			TF2Attrib_SetByName(Weapon1, "aiming movespeed decreased", 1.4);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBBHEAVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		TF2Attrib_RemoveByName(Weapon1, "damage bonus");
		TF2Attrib_RemoveByName(Weapon1, "aiming movespeed decreased");
		TF2Attrib_RemoveByName(Weapon1, "maxammo primary increased");	
		TF2Attrib_RemoveByName(Weapon1, "killstreak tier");
	}
} 

public Action:GBrassBeastHeavySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBBHEAVY[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGBBHEAVY[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGBrassBeastHeavy(Handle:plugin, args)
        MakeGBrassBeastHeavy(GetNativeCell(1));
 
public Native_IsGBrassBeastHeavy(Handle:plugin, args)
        return g_bIsGBBHEAVY[GetNativeCell(1)];*/

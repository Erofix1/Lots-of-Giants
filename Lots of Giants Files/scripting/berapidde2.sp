...
#define RAPIDDEMO2				"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Giant Rapid Fire Demoman (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Rapid Fire Demoman (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsRAPIDDEMO2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("berapidde2_version", PLUGIN_VERSION, "[TF2] Be the Giant Rapid Fire Demoman (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_berapidde2", Command_RapidDemo2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(RapidDemo2SH);
...
{
//	CreateNative("BeRapidDemo2_MakeRapidDemo2", Native_SetRapidDemo2);
//	CreateNative("BeRapidDemo2_IsRapidDemo2", Native_IsRapidDemo2);
...
{
	if (g_bIsRAPIDDEMO2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsRAPIDDEMO2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(RAPIDDEMO2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsRAPIDDEMO2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_RapidDemo2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeRapidDemo2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Rapid Fire Demoman (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Rapid Fire Demoman (Type 2) !");
		PrintToChat(target_list[i], "2. Type 2 has less Health, faster fire rate but no faster reload speed and bonus clip size.");
...
MakeRapidDemo2(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 2825.0);
...
{
	if (IsValidClient(client))
			GiveRapidDemo2(client);
}
 
stock GiveRapidDemo2(client)
{
	if (IsValidClient(client))
	{
		g_bIsRAPIDDEMO2[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsRAPIDDEMO2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:RapidDemo2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsRAPIDDEMO2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetRapidDemo2(Handle:plugin, args)
        MakeRapidDemo2(GetNativeCell(1));
 
public Native_IsRapidDemo2(Handle:plugin, args)
        return g_bIsRAPIDDEMO2[GetNativeCell(1)];*/

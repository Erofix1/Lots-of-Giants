...
#define GFLAREPYRO2     "models/bots/pyro_boss/bot_pyro_boss.mdl"
...
{
	name = "[TF2] Be the Giant Flare Pyro (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Flare Pyro (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGFLAREPYRO2[MAXPLAYERS + 1];
 
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beflarepyro2_version", PLUGIN_VERSION, "[TF2] Be the Unused Giant Flare Pyro (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beflarepyro2", Command_GiantFlarePyro2, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantFlarePyro2SH);
...
{
//	CreateNative("BeGiantFlarePyro2_MakeGiantFlarePyro2", Native_SetGiantFlarePyro2);
//	CreateNative("BeGiantFlarePyro2_IsGiantFlarePyro2", Native_IsGiantFlarePyro2);
...
{
	if (g_bIsGFLAREPYRO2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		
		g_bIsGFLAREPYRO2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GFLAREPYRO2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGFLAREPYRO2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantFlarePyro2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveGiantFlarePyro2(client);
}
 
stock GiveGiantFlarePyro2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGFLAREPYRO2[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGFLAREPYRO2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
...
public Action:GiantFlarePyro2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGFLAREPYRO2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantFlarePyro2(Handle:plugin, args)
        MakeGiantFlarePyro2(GetNativeCell(1));
 
public Native_IsGiantFlarePyro2(Handle:plugin, args)
        return g_bIsGFLAREPYRO2[GetNativeCell(1)];*/

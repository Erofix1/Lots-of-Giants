...
#define CHIEFBLASTSOL2             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Chief Blast Soldier (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Chief Blast Soldier (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFBLASTSOL2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefblastsol2_version", PLUGIN_VERSION, "[TF2] Be the Chief Blast Soldier (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefblastsol2", Command_ChiefBlastSol2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefBlastSol2SH);
...
{
//	CreateNative("BeChiefBlastSol2_MakeChiefBlastSol2", Native_SetChiefBlastSol2);
//	CreateNative("BeChiefBlastSol2_IsChiefBlastSol2", Native_IsChiefBlastSol2);
...
{
	if (g_bIsCHIEFBLASTSOL2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFBLASTSOL2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFBLASTSOL2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFBLASTSOL2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefBlastSol2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeChiefBlastSol2(client)
{
	...
	SetModel(client, CHIEFBLASTSOL2);
   
	TF2_SetHealth(client, 58000);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 57800.0);
	...
	g_bIsCHIEFBLASTSOL2[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveChiefBlastSol2(client);
}
 
stock GiveChiefBlastSol2(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFBLASTSOL2[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFBLASTSOL2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChiefBlastSol2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFBLASTSOL2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefBlastSol2(Handle:plugin, args)
        MakeChiefBlastSol2(GetNativeCell(1));
 
public Native_IsChiefBlastSol2(Handle:plugin, args)
        return g_bIsCHIEFBLASTSOL2[GetNativeCell(1)];*/

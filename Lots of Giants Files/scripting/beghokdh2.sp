...
#define GHOKDH2      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Heal-on-Kill Deflector Heavy (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Heal-on-Kill Deflector Heavy (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGHOKDH2[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("beghokdh2_version", PLUGIN_VERSION, "[TF2] Be the Giant Heal-on-Kill Deflector Heavy (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beghokdh2", Command_Ghokdh2, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(Ghokdh2SH);
...
{
//	CreateNative("BeGhokdh2_MakeGhokdh2", Native_SetGhokdh2);
//	CreateNative("BeGhokdh2_IsGhokdh2", Native_IsGhokdh2);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGHOKDH2[client])
	{
		...
		g_bIsGHOKDH2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHOKDH2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHOKDH2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_Ghokdh2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGhokdh2(client)
{
	...
	SetModel(client, GHOKDH2);

	TF2_SetHealth(client, 5500);
	...
	TF2Attrib_SetByName(client, "rage giving scale", 0.9);
	TF2Attrib_SetByName(client, "damage force reduction", 0.3);
	TF2Attrib_SetByName(client, "airblast vulnerability multiplier", 0.4);
	TF2Attrib_SetByName(client, "health from packs decreased", 0.0);
	TF2Attrib_SetByName(client, "aiming movespeed increased", 2.0);
	TF2Attrib_SetByName(client, "max health additive bonus", 5200.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHOKDH2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:Ghokdh2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHOKDH2[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGHOKDH2[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGhokdh2(Handle:plugin, args)
        MakeGhokdh2(GetNativeCell(1));
 
public Native_IsGhokdh2(Handle:plugin, args)
        return g_bIsGHOKDH2[GetNativeCell(1)];*/

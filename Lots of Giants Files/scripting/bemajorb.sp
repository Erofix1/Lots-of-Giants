...
#define MAJORBOMBER		"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Major Bomber",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Major Bomber from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsMAJORBOMBER[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bemajorb_version", PLUGIN_VERSION, "[TF2] Be the Major Bomber version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bemajorb", Command_MajorBomber, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(MajorBomberSH);
...
{
//	CreateNative("BeMajorBomber_MakeMajorBomber", Native_SetMajorBomber);
//	CreateNative("BeMajorBomber_IsMajorBomber", Native_IsMajorBomber);
...
{
	if (g_bIsMAJORBOMBER[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsMAJORBOMBER[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(MAJORBOMBER);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsMAJORBOMBER[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_MajorBomber(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeMajorBomber(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 39825.0);
	TF2Attrib_SetByName(client, "ammo regen", 100.0);
	TF2Attrib_SetByName(client, "health regen", 200.0);
	TF2Attrib_SetByName(client, "damage force reduction", 0.7);
	TF2Attrib_SetByName(client, "move speed penalty", 0.32);
...
{
	if (IsValidClient(client))
			GiveMajorBomber(client);
}
 
stock GiveMajorBomber(client)
{
	if (IsValidClient(client))
	{
		g_bIsMAJORBOMBER[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsMAJORBOMBER[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:MajorBomberSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsMAJORBOMBER[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetMajorBomber(Handle:plugin, args)
        MakeMajorBomber(GetNativeCell(1));
 
public Native_IsMajorBomber(Handle:plugin, args)
        return g_bIsMAJORBOMBER[GetNativeCell(1)];*/

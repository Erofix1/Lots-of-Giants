...
#define GDEMO				"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Giant Demoman",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Demoman from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGDEMO[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begde_version", PLUGIN_VERSION, "[TF2] Be the Giant Demoman version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begde", Command_GiantDemo, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantDemoSH);
...
{
//	CreateNative("BeGiantDemo_MakeGiantDemo", Native_SetGiantDemo);
//	CreateNative("BeGiantDemo_IsGiantDemo", Native_IsGiantDemo);
...
{
	if (g_bIsGDEMO[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGDEMO[client] = false;
	}
}
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGDEMO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantDemo(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveGiantDemo(client);
}
 
stock GiveGiantDemo(client)
{
	if (IsValidClient(client))
	{
		g_bIsGDEMO[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGDEMO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantDemoSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGDEMO[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantDemo(Handle:plugin, args)
        MakeGiantDemo(GetNativeCell(1));
 
public Native_IsGiantDemo(Handle:plugin, args)
        return g_bIsGDEMO[GetNativeCell(1)];*/

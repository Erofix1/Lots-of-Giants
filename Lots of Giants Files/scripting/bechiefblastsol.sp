...
#define CHIEFBLASTSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Chief Blast Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Chief Blast Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFBLASTSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefblastsol_version", PLUGIN_VERSION, "[TF2] Be the Chief Blast Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefblastsol", Command_ChiefBlastSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefBlastSolSH);
...
{
//	CreateNative("BeChiefBlastSol_MakeChiefBlastSol", Native_SetChiefBlastSol);
//	CreateNative("BeChiefBlastSol_IsChiefBlastSol", Native_IsChiefBlastSol);
...
{
	if (g_bIsCHIEFBLASTSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFBLASTSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFBLASTSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFBLASTSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefBlastSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeChiefBlastSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Chief Blast Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Chief Blast Soldier !");
...
MakeChiefBlastSol(client)
{
	...
	SetModel(client, CHIEFBLASTSOL);
   
	TF2_SetHealth(client, 35000);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 34800.0);
	...
	g_bIsCHIEFBLASTSOL[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveChiefBlastSol(client);
}
 
stock GiveChiefBlastSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFBLASTSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFBLASTSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChiefBlastSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFBLASTSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefBlastSol(Handle:plugin, args)
        MakeChiefBlastSol(GetNativeCell(1));
 
public Native_IsChiefBlastSol(Handle:plugin, args)
        return g_bIsCHIEFBLASTSOL[GetNativeCell(1)];*/

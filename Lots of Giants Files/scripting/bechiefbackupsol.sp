...
#define CHIEFBACKUPSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Chief Battalion's Backup Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Chief Battalion's Backup Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFBACKUPSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefbackupsol_version", PLUGIN_VERSION, "[TF2] Be the Chief Battalion's Backup Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefbackupsol", Command_ChiefBattalionsBackupSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ChiefBattalionsBackupSolSH);
...
{
//	CreateNative("BeChiefBattalionsBackupSol_MakeChiefBattalionsBackupSol", Native_SetChiefBattalionsBackupSol);
//	CreateNative("BeChiefBattalionsBackupSol_IsChiefBattalionsBackupSol", Native_IsChiefBattalionsBackupSol);
...
{
	if (g_bIsCHIEFBACKUPSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFBACKUPSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFBACKUPSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFBACKUPSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefBattalionsBackupSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeChiefBattalionsBackupSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Chief Battalion's Backup Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Chief Battalion's Backup Soldier !");
		PrintToChat(target_list[i], "2. Your Battalion's Backup starts with Full RAGE and lasts much longer than normal !");
		PrintToChat(target_list[i], "3. Chief has more health than Giant Battalion's Backup Soldier !");
...
MakeChiefBattalionsBackupSol(client)
{
	...
	SetModel(client, CHIEFBACKUPSOL);
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 49780.0);
	...
	g_bIsCHIEFBACKUPSOL[client] = true;
...
{
	if (IsValidClient(client))
			GiveChiefBattalionsBackupSol(client);
}
 
stock GiveChiefBattalionsBackupSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFBACKUPSOL[client] = true;
		...
		CreateWeapon(client, "tf_weapon_buff_item", 226, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 227, 10, 6);
		...
		while ((iEntity = FindEntityByClassname(iEntity, "tf_weapon_buff_item")) != -1)
		{
			...
			{
				TF2Attrib_SetByName(iEntity, "increase buff duration", 91.0);				
				break;
			}
		}
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFBACKUPSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChiefBattalionsBackupSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFBACKUPSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefBattalionsBackupSol(Handle:plugin, args)
        MakeChiefBattalionsBackupSol(GetNativeCell(1));
 
public Native_IsChiefBattalionsBackupSol(Handle:plugin, args)
        return g_bIsCHIEFBACKUPSOL[GetNativeCell(1)];*/

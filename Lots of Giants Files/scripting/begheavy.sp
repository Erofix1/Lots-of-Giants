...
#define GHEAVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGHEAVY[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("begheavy_version", PLUGIN_VERSION, "[TF2] Be the Giant Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begheavy", Command_GHeavy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GHeavySH);
...
{
//	CreateNative("BeGHeavy_MakeGHeavy", Native_SetGHeavy);
//	CreateNative("BeGHeavy_IsGHeavy", Native_IsGHeavy);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGHEAVY[client])
	{
		...
		g_bIsGHEAVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHEAVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHEAVY[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_GHeavy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
stock GiveGHeavy(client)
{
	if (IsValidClient(client))
	{
		...
		CreateWeapon(client, "tf_weapon_minigun", 15, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHEAVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GHeavySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHEAVY[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGHEAVY[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGHeavy(Handle:plugin, args)
        MakeGHeavy(GetNativeCell(1));
 
public Native_IsGHeavy(Handle:plugin, args)
        return g_bIsGHEAVY[GetNativeCell(1)];*/

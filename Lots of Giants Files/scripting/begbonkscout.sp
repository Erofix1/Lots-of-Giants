...
#define GBONKSCOUT		"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Giant Bonk Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Bonk Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBONKSCOUT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begbonkscout_version", PLUGIN_VERSION, "[TF2] Be the Giant Bonk Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begbonkscout", Command_GiantBonkScout, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(GiantBonkScoutSH);
...
{
//	CreateNative("BeGiantBonkScout_MakeGiantBonkScout", Native_SetGiantBonkScout);
//	CreateNative("BeGiantBonkScout_IsGiantBonkScout", Native_IsGiantBonkScout);
...
{
	if (g_bIsGBONKSCOUT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBONKSCOUT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBONKSCOUT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBONKSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBonkScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBonkScout(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Bonk Scout !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Bonk Scout !");
		PrintToChat(target_list[i], "2. Your Bonk! Atomic Punch recharges faster than normal.");
...
{
	if (IsValidClient(client))
			GiveGiantBonkScout(client);
}
 
stock GiveGiantBonkScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBONKSCOUT[client] = true;
		...
		CreateWeapon(client, "tf_weapon_lunchbox_drink", 46, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		CreateWeapon(client, "tf_weapon_bat", 0, 6, 1, 2, 0);

		CreateHat(client, 106, 10, 6);
		...
		if(IsValidEntity(Weapon2))
		{
			...
			TF2Attrib_SetByName(Weapon2, "effect bar recharge rate increased", 0.55);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBONKSCOUT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
		TF2Attrib_RemoveByName(Weapon2, "effect bar recharge rate increased");	
	}
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:GiantBonkScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBONKSCOUT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBonkScout(Handle:plugin, args)
        MakeGiantBonkScout(GetNativeCell(1));
 
public Native_IsGiantBonkScout(Handle:plugin, args)
        return g_bIsGBONKSCOUT[GetNativeCell(1)];*/

...
#define CHIEFMLSCOUT		"models/bots/scout_boss/bot_scout_boss.mdl"
...
{
	name = "[TF2] Be the Chief Major League Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Chief Major League Scout from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCHIEFMLSCOUT[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("bechiefmlscout_version", PLUGIN_VERSION, "[TF2] Be the Chief Major League Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechiefmlscout", Command_ChiefMajorLeagueScout, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(ChiefMajorLeagueScoutSH);
...
{
//	CreateNative("BeChiefMajorLeagueScout_MakeChiefMajorLeagueScout", Native_SetChiefMajorLeagueScout);
//	CreateNative("BeChiefMajorLeagueScout_IsChiefMajorLeagueScout", Native_IsChiefMajorLeagueScout);
...
{
	if (g_bIsCHIEFMLSCOUT[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCHIEFMLSCOUT[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHIEFMLSCOUT);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHIEFMLSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ChiefMajorLeagueScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeChiefMajorLeagueScout(client)
{
	...
	SetModel(client, CHIEFMLSCOUT);
   
	TF2_SetHealth(client, 10000);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", true);
	TF2Attrib_SetByName(client, "max health additive bonus", 9890.0);
	TF2Attrib_SetByName(client, "move speed bonus", 1.3);
	TF2Attrib_SetByName(client, "damage force reduction", 0.7);
	TF2Attrib_SetByName(client, "airblast vulnerability multiplier", 3.0);
...
{
	if (IsValidClient(client))
			GiveChiefMajorLeagueScout(client);
}
 
stock GiveChiefMajorLeagueScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsCHIEFMLSCOUT[client] = true;
		...
		CreateWeapon(client, "tf_weapon_bat_wood", 44, 6, 1, 2, 0);

		CreateHat(client, 942, 10, 6);
		CreateHat(client, 486, 10, 6);
		CreateHat(client, 707, 10, 6);
		
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
		if(IsValidEntity(Weapon3))
		{
			TF2Attrib_RemoveAll(Weapon3);
			TF2Attrib_SetByName(Weapon3, "damage bonus", 3.0);
			TF2Attrib_SetByName(Weapon3, "effect bar recharge rate increased", 0.001);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHIEFMLSCOUT[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
		TF2Attrib_RemoveByName(Weapon3, "damage bonus");
		TF2Attrib_RemoveByName(Weapon3, "effect bar recharge rate increased");
		TF2Attrib_RemoveByName(Weapon3, "killstreak tier");
	}
}
 
public Action:ChiefMajorLeagueScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHIEFMLSCOUT[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChiefMajorLeagueScout(Handle:plugin, args)
        MakeChiefMajorLeagueScout(GetNativeCell(1));
 
public Native_IsChiefMajorLeagueScout(Handle:plugin, args)
        return g_bIsCHIEFMLSCOUT[GetNativeCell(1)];*/

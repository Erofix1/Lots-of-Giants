...
#define GSHOTGUNH      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Shotgun Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Shotgun Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGSHOTGUNH[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begshotgunh_version", PLUGIN_VERSION, "[TF2] Be the Giant Shotgun Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begshotgunh", Command_GShotgunH, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GShotgunHSH);
...
{
//	CreateNative("BeGShotgunH_MakeGShotgunH", Native_SetGShotgunH);
//	CreateNative("BeGShotgunH_IsGShotgunH", Native_IsGShotgunH);
...
{
	if (g_bIsGSHOTGUNH[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGSHOTGUNH[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GSHOTGUNH);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGSHOTGUNH[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GShotgunH(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGShotgunH(client)
{
	...
	{
		SetEntProp(GetPlayerWeaponSlot(client, 1), Prop_Send, "m_iWeaponState", 0);
		TF2_RemoveCondition(client, TFCond_Slowed);
	}
	CreateTimer(0.0, Timer_Switch, client);
	SetModel(client, GSHOTGUNH);
...
{
	if (IsValidClient(client))
		GiveGShotgunH(client);
}
 
stock GiveGShotgunH(client)
{
	if (IsValidClient(client))
	{
		g_bIsGSHOTGUNH[client] = true;
		...
		CreateWeapon(client, "tf_weapon_shotgun_hwg", 11, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGSHOTGUNH[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
			
		TF2Attrib_RemoveByName(Weapon2, "fire rate penalty");
		TF2Attrib_RemoveByName(Weapon2, "attack projectiles");
		TF2Attrib_RemoveByName(Weapon2, "bullets per shot bonus");
		TF2Attrib_RemoveByName(Weapon2, "damage penalty");
...
public Action:GShotgunHSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGSHOTGUNH[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGShotgunH(Handle:plugin, args)
        MakeGShotgunH(GetNativeCell(1));
 
public Native_IsGShotgunH(Handle:plugin, args)
        return g_bIsGSHOTGUNH[GetNativeCell(1)];*/

...
#define GFLAREPYRO     "models/bots/pyro_boss/bot_pyro_boss.mdl"
...
{
	name = "[TF2] Be the Giant Flare Pyro",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Flare Pyro from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGFLAREPYRO[MAXPLAYERS + 1];
 
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beflarepyro_version", PLUGIN_VERSION, "[TF2] Be the Giant Flare Pyro version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beflarepyro", Command_GiantFlarePyro, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantFlarePyroSH);
...
{
//	CreateNative("BeGiantFlarePyro_MakeGiantFlarePyro", Native_SetGiantFlarePyro);
//	CreateNative("BeGiantFlarePyro_IsGiantFlarePyro", Native_IsGiantFlarePyro);
...
{
	if (g_bIsGFLAREPYRO[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		
		g_bIsGFLAREPYRO[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GFLAREPYRO);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGFLAREPYRO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantFlarePyro(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantFlarePyro(client)
{
	...
	{
		SetEntProp(GetPlayerWeaponSlot(client, 1), Prop_Send, "m_iWeaponState", 1);
		TF2_RemoveCondition(client, TFCond_Slowed);
	}
	CreateTimer(0.0, Timer_Switch, client);
	SetModel(client, GFLAREPYRO);
...
{
	if (IsValidClient(client))
		GiveGiantFlarePyro(client);
}
 
stock GiveGiantFlarePyro(client)
{
	if (IsValidClient(client))
	{
		g_bIsGFLAREPYRO[client] = true;
		...
		CreateWeapon(client, "tf_weapon_flaregun", 351, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);

		CreateHat(client, 247, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGFLAREPYRO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
		TF2Attrib_RemoveByName(Weapon2, "fire rate bonus");
		TF2Attrib_RemoveByName(Weapon2, "maxammo secondary increased");	
		TF2Attrib_RemoveByName(Weapon2, "killstreak tier");
	}
}
 
public Action:GiantFlarePyroSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGFLAREPYRO[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantFlarePyro(Handle:plugin, args)
        MakeGiantFlarePyro(GetNativeCell(1));
 
public Native_IsGiantFlarePyro(Handle:plugin, args)
        return g_bIsGFLAREPYRO[GetNativeCell(1)];*/

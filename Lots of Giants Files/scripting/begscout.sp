...
{
	name = "[TF2] Be the Giant Scout",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Scout from MvM",
...
public OnPluginStart()
{
	...
	CreateConVar("begscout_version", PLUGIN_VERSION, "[TF2] Be the Giant Scout version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begscout", Command_GiantScout, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantScoutSH);
...
{
//	CreateNative("BeGiantScout_MakeGiantScout", Native_SetGiantScout);
//	CreateNative("BeGiantScout_IsGiantScout", Native_IsGiantScout);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGSCOUT[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantScout(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveGiantScout(client);
}
 
stock GiveGiantScout(client)
{
	if (IsValidClient(client))
	{
		g_bIsGSCOUT[client] = true;
...
public Action:GiantScoutSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantScout(Handle:plugin, args)
        MakeGiantScout(GetNativeCell(1));
 
public Native_IsGiantScout(Handle:plugin, args)

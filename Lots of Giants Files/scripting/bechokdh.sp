...
#define CHOKDH      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Chief Heal-on-Kill Deflector Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Chief Heal-on-Kill Deflector Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsCHOKDH[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("bechokdh_version", PLUGIN_VERSION, "[TF2] Be the Chief Heal-on-Kill Deflector Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_bechokdh", Command_Chokdh, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(ChokdhSH);
...
{
//	CreateNative("BeChokdh_MakeChokdh", Native_SetChokdh);
//	CreateNative("BeChokdh_IsChokdh", Native_IsChokdh);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsCHOKDH[client])
	{
		...
		g_bIsCHOKDH[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(CHOKDH);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCHOKDH[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_Chokdh(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeChokdh(client)
{
	...
	SetModel(client, CHOKDH);
   
	TF2_SetHealth(client, 70000);
	...
	TF2Attrib_SetByName(client, "rage giving scale", 0.6);
	TF2Attrib_SetByName(client, "damage force reduction", 0.3);
	TF2Attrib_SetByName(client, "airblast vulnerability multiplier", 0.4);
	TF2Attrib_SetByName(client, "health from packs decreased", 0.0);
	TF2Attrib_SetByName(client, "aiming movespeed increased", 2.0);
	TF2Attrib_SetByName(client, "max health additive bonus", 69700.0);
...
stock GiveChokdh(client)
{
	if (IsValidClient(client))
	{
		...
		CreateHat(client, 97, 10, 6);
		CreateHat(client, 392, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "heal on kill", 8000.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCHOKDH[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ChokdhSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCHOKDH[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsCHOKDH[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetChokdh(Handle:plugin, args)
        MakeChokdh(GetNativeCell(1));
 
public Native_IsChokdh(Handle:plugin, args)
        return g_bIsCHOKDH[GetNativeCell(1)];*/

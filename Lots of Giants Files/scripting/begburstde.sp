...
#define GBURSTDEMO				"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Giant Burst Fire Demoman",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Burst Fire Demoman from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBURSTDEMO[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begburstde_version", PLUGIN_VERSION, "[TF2] Be the Giant Burst Fire Demoman version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begburstde", Command_BurstDemo, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(BurstDemoSH);
...
{
//	CreateNative("BeBurstDemo_MakeBurstDemo", Native_SetBurstDemo);
//	CreateNative("BeBurstDemo_IsBurstDemo", Native_IsBurstDemo);
...
{
	if (g_bIsGBURSTDEMO[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBURSTDEMO[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBURSTDEMO);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBURSTDEMO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_BurstDemo(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeBurstDemo(client)
{
	...
	SetModel(client, GBURSTDEMO);
   
	TF2_SetHealth(client, 3300);
   
	SetEntPropFloat(client, Prop_Send, "m_flModelScale", 1.75);
	SetEntProp(client, Prop_Send, "m_bIsMiniBoss", _:true);
	TF2Attrib_SetByName(client, "health from packs decreased", 0.0);
	TF2Attrib_SetByName(client, "max health additive bonus", 3125.0);
...
{
	if (IsValidClient(client))
			GiveBurstDemo(client);
}
 
stock GiveBurstDemo(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBURSTDEMO[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBURSTDEMO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:BurstDemoSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBURSTDEMO[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetBurstDemo(Handle:plugin, args)
        MakeBurstDemo(GetNativeCell(1));
 
public Native_IsBurstDemo(Handle:plugin, args)
        return g_bIsGBURSTDEMO[GetNativeCell(1)];*/

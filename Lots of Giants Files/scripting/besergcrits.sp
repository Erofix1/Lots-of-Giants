...
#define SERGEANTCRITS          "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be Sergeant Crits",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as Sergeant Crits from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsSERGEANTCRITS[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("besergcrits_version", PLUGIN_VERSION, "[TF2] Be Sergeant Crits version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_besergcrits", Command_SergeantCrits, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(SergeantCritsSH);
...
{
//	CreateNative("BeSergeantCrits_MakeSergeantCrits", Native_SetSergeantCrits);
//	CreateNative("BeSergeantCrits_IsSergeantCrits", Native_IsSergeantCrits);
...
{
	if (g_bIsSERGEANTCRITS[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsSERGEANTCRITS[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(SERGEANTCRITS);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSERGEANTCRITS[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SergeantCrits(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeSergeantCrits(client)
{
	...
	g_bIsSERGEANTCRITS[client] = true;
}
...
{
	if (IsValidClient(client))
			GiveSergeantCrits(client);
}
 
stock GiveSergeantCrits(client)
{
	if (IsValidClient(client))
	{
		g_bIsSERGEANTCRITS[client] = true;
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "projectile speed increased", 1.30);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSERGEANTCRITS[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:SergeantCritsSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsSERGEANTCRITS[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetSergeantCrits(Handle:plugin, args)
        MakeSergeantCrits(GetNativeCell(1));
 
public Native_IsSergeantCrits(Handle:plugin, args)
        return g_bIsSERGEANTCRITS[GetNativeCell(1)];*/

...
#define GHEATHEAVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Heater Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Heater Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGHEATHEAVY[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("begheaterheavy_version", PLUGIN_VERSION, "[TF2] Be the Giant Heater Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begheaterheavy", Command_GiantHeaterHeavy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantHeaterHeavySH);
...
{
//	CreateNative("BeGiantHeaterHeavy_MakeGiantHeaterHeavy", Native_SetGiantHeaterHeavy);
//	CreateNative("BeGiantHeaterHeavy_IsGiantHeaterHeavy", Native_IsGiantHeaterHeavy);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGHEATHEAVY[client])
	{
		...
		g_bIsGHEATHEAVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHEATHEAVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHEATHEAVY[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_GiantHeaterHeavy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveGiantHeaterHeavy(client);
}
 
stock GiveGiantHeaterHeavy(client)
{
	if (IsValidClient(client))
	{
		g_bIsGHEATHEAVY[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHEATHEAVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantHeaterHeavySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHEATHEAVY[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGHEATHEAVY[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantHeaterHeavy(Handle:plugin, args)
        MakeGiantHeaterHeavy(GetNativeCell(1));
 
public Native_IsGiantHeaterHeavy(Handle:plugin, args)
        return g_bIsGHEATHEAVY[GetNativeCell(1)];*/

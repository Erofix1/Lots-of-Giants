...
#define RAPIDDEMO				"models/bots/demo_boss/bot_demo_boss.mdl"
...
{
	name = "[TF2] Be the Giant Rapid Fire Demoman",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Rapid Fire Demoman from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsRAPIDDEMO[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("berapidde_version", PLUGIN_VERSION, "[TF2] Be the Giant Rapid Fire Demoman version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_berapidde", Command_RapidDemo, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(RapidDemoSH);
...
{
//	CreateNative("BeRapidDemo_MakeRapidDemo", Native_SetRapidDemo);
//	CreateNative("BeRapidDemo_IsRapidDemo", Native_IsRapidDemo);
...
{
	if (g_bIsRAPIDDEMO[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsRAPIDDEMO[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(RAPIDDEMO);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsRAPIDDEMO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_RapidDemo(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveRapidDemo(client);
}
 
stock GiveRapidDemo(client)
{
	if (IsValidClient(client))
	{
		g_bIsRAPIDDEMO[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsRAPIDDEMO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:RapidDemoSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsRAPIDDEMO[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetRapidDemo(Handle:plugin, args)
        MakeRapidDemo(GetNativeCell(1));
 
public Native_IsRapidDemo(Handle:plugin, args)
        return g_bIsRAPIDDEMO[GetNativeCell(1)];*/

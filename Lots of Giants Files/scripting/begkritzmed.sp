...
#define GKRITZMEDIC             "models/bots/medic/bot_medic.mdl"
...
{
	name = "[TF2] Be the Giant Kritzkrieg Medic",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Kritzkrieg Medic from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGKRITZMEDIC[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begkritzmed_version", PLUGIN_VERSION, "[TF2] Be the Giant Kritzkrieg Medic version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begkritzmed", Command_GiantKritzkriegMedic, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantKritzkriegMedicSH);
...
{
//	CreateNative("BeGiantKritzkriegMedic_MakeGiantKritzkriegMedic", Native_SetGiantKritzkriegMedic);
//	CreateNative("BeGiantKritzkriegMedic_IsGiantKritzkriegMedic", Native_IsGiantKritzkriegMedic);
...
{
	if (g_bIsGKRITZMEDIC[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGKRITZMEDIC[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GKRITZMEDIC);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGKRITZMEDIC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantKritzkriegMedic(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantKritzkriegMedic(client)
{
...
{
	if (IsValidClient(client))
			GiveGiantKritzkriegMedic(client);
}
 
stock GiveGiantKritzkriegMedic(client)
{
	if (IsValidClient(client))
	{
		g_bIsGKRITZMEDIC[client] = true;
		...
		if(IsValidEntity(Weapon2))
		{
			...
			TF2Attrib_SetByName(Weapon2, "uber duration bonus", 9999.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGKRITZMEDIC[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
		TF2Attrib_RemoveByName(Weapon2, "heal rate bonus");
		TF2Attrib_RemoveByName(Weapon2, "uber duration bonus");
		TF2Attrib_RemoveByName(Weapon2, "killstreak tier");	
	}
}

 
public Action:GiantKritzkriegMedicSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGKRITZMEDIC[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantKritzkriegMedic(Handle:plugin, args)
        MakeGiantKritzkriegMedic(GetNativeCell(1));
 
public Native_IsGiantKritzkriegMedic(Handle:plugin, args)
        return g_bIsGKRITZMEDIC[GetNativeCell(1)];*/

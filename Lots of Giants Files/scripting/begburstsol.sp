...
#define GBURSTSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Burst Fire Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Burst Fire Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBURSTSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begburstsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Burst Fire Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begburstsol", Command_GiantBurstFireSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBurstFireSolSH);
...
{
//	CreateNative("BeGiantBurstFireSol_MakeGiantBurstFireSol", Native_SetGiantBurstFireSol);
//	CreateNative("BeGiantBurstFireSol_IsGiantBurstFireSol", Native_IsGiantBurstFireSol);
...
{
	if (g_bIsGBURSTSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBURSTSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBURSTSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBURSTSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBurstFireSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBurstFireSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Burst Fire Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Burst Fire Soldier !");
...
{
	if (IsValidClient(client))
			GiveGiantBurstFireSol(client);
}
 
stock GiveGiantBurstFireSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBURSTSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBURSTSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBurstFireSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBURSTSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBurstFireSol(Handle:plugin, args)
        MakeGiantBurstFireSol(GetNativeCell(1));
 
public Native_IsGiantBurstFireSol(Handle:plugin, args)
        return g_bIsGBURSTSOL[GetNativeCell(1)];*/

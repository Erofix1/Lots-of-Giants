...
#define GNATHEAVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Natascha Heavy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Natascha Heavy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGNATHEAVY[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("begnatheavy_version", PLUGIN_VERSION, "[TF2] Be the Giant Natascha Heavy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begnatheavy", Command_GiantNataschaHeavy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantNataschaHeavySH);
...
{
//	CreateNative("BeGiantNataschaHeavy_MakeGiantNataschaHeavy", Native_SetGiantNataschaHeavy);
//	CreateNative("BeGiantNataschaHeavy_IsGiantNataschaHeavy", Native_IsGiantNataschaHeavy);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGNATHEAVY[client])
	{
		...
		g_bIsGNATHEAVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GNATHEAVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGNATHEAVY[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_GiantNataschaHeavy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantNataschaHeavy(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Natascha Heavy !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Natascha Heavy !");
		PrintToChat(target_list[i], "2. Your Natascha slows enemies and deals 25 percent more damage !");
...
{
	if (IsValidClient(client))
		GiveGiantNataschaHeavy(client);
}
 
stock GiveGiantNataschaHeavy(client)
{
	if (IsValidClient(client))
	{
		g_bIsGNATHEAVY[client] = true;
		...
		CreateWeapon(client, "tf_weapon_minigun", 41, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGNATHEAVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantNataschaHeavySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGNATHEAVY[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGNATHEAVY[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantNataschaHeavy(Handle:plugin, args)
        MakeGiantNataschaHeavy(GetNativeCell(1));
 
public Native_IsGiantNataschaHeavy(Handle:plugin, args)
        return g_bIsGNATHEAVY[GetNativeCell(1)];*/

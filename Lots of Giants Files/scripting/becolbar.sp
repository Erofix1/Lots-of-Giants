...
#define COLBAR             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Colonel Barrage",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Colonel Barrage from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsCOLBAR[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("becolbar_version", PLUGIN_VERSION, "[TF2] Be the Colonel Barrage version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_becolbar", Command_ColonelBarrage, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(ColonelBarrageSH);
...
{
//	CreateNative("BeColonelBarrage_MakeColonelBarrage", Native_SetColonelBarrage);
//	CreateNative("BeColonelBarrage_IsColonelBarrage", Native_IsColonelBarrage);
...
{
	if (g_bIsCOLBAR[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsCOLBAR[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(COLBAR);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsCOLBAR[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_ColonelBarrage(client, args)
{
	...
	{
		MakeColonelBarrage(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Colonel Barrage !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Colonel Barrage !");
		PrintToChat(target_list[i], "2. Unleash a Barrage of high damage, slow moving Rockets !");
...
{
	if (IsValidClient(client))
			GiveColonelBarrage(client);
}
 
stock GiveColonelBarrage(client)
{
	if (IsValidClient(client))
	{
		g_bIsCOLBAR[client] = true;
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "faster reload rate", 0.22);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsCOLBAR[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:ColonelBarrageSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsCOLBAR[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetColonelBarrage(Handle:plugin, args)
        MakeColonelBarrage(GetNativeCell(1));
 
public Native_IsColonelBarrage(Handle:plugin, args)
        return g_bIsCOLBAR[GetNativeCell(1)];*/

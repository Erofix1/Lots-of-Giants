...
#define GHOKDH3      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Heal-on-Kill Deflector Heavy (Type 3)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Heal-on-Kill Deflector Heavy (Type 3) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGHOKDH3[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("beghokdh3_version", PLUGIN_VERSION, "[TF2] Be the Giant Heal-on-Kill Deflector Heavy (Type 3) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beghokdh3", Command_Ghokdh3, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(Ghokdh3SH);
...
{
//	CreateNative("BeGhokdh3_MakeGhokdh3", Native_SetGhokdh3);
//	CreateNative("BeGhokdh3_IsGhokdh3", Native_IsGhokdh3);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGHOKDH3[client])
	{
		...
		g_bIsGHOKDH3[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHOKDH3);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHOKDH3[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_Ghokdh3(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGhokdh3(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Heal-on-Kill Deflector Heavy (Type 3) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Heal-on-Kill Deflector Heavy (Type 3) !");
		PrintToChat(target_list[i], "2. Type 3 is no longer a GateBot and has even less max. health.");
...
stock GiveGhokdh3(client)
{
	if (IsValidClient(client))
	{
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "heal on kill", 5000.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHOKDH3[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		TF2Attrib_RemoveByName(Weapon1, "damage bonus");
		TF2Attrib_RemoveByName(Weapon1, "attack projectiles");
		TF2Attrib_RemoveByName(Weapon1, "heal on kill");
		TF2Attrib_RemoveByName(Weapon1, "maxammo primary increased");
		TF2Attrib_RemoveByName(Weapon1, "killstreak tier");		
	}
}
 
public Action:Ghokdh3SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHOKDH3[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGHOKDH3[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGhokdh3(Handle:plugin, args)
        MakeGhokdh3(GetNativeCell(1));
 
public Native_IsGhokdh3(Handle:plugin, args)
        return g_bIsGHOKDH3[GetNativeCell(1)];*/

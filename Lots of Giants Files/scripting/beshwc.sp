...
new bool:g_bIsSHWC[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beshwc_version", PLUGIN_VERSION, "[TF2] Be the Super Heavyweight Champ version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beshwc", Command_SuperHeavyweightChamp, ADMFLAG_ROOT, "It's a good time to run");
...
{
//	CreateNative("BeSuperHeavyweightChamp_MakeSuperHeavyweightChamp", Native_SetSuperHeavyweightChamp);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSHWC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SuperHeavyweightChamp(client, args)
{
	...
	{
		MakeSuperHeavyweightChamp(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Super Heavyweight Champ !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Super Heavyweight Champ !");
		PrintToChat(target_list[i], "2. Melee only but you attack faster and deal more damage.");
...
MakeSuperHeavyweightChamp(client)
{
...
{
	if (IsValidClient(client))
			GiveSuperHeavyweightChamp(client);
}
 
stock GiveSuperHeavyweightChamp(client)
{
	if (IsValidClient(client))
	{
		g_bIsSHWC[client] = true;
		...
		if(IsValidEntity(Weapon3))
		{
			...
			TF2Attrib_SetByName(Weapon3, "critboost on kill", 5.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSHWC[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:SuperHeavyweightChampSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (!g_bIsSHWC[entity]) return Plugin_Continue;
...
public Native_SetSuperHeavyweightChamp(Handle:plugin, args)
        MakeSuperHeavyweightChamp(GetNativeCell(1));
 
public Native_IsSuperHeavyweightChamp(Handle:plugin, args)
        return g_bIsSHWC[GetNativeCell(1)];*/

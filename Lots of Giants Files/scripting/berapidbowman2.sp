...
#define RAPIDBOWMAN2		"models/bots/sniper/bot_sniper.mdl"
...
{
	name = "[TF2] Be the Rapid Fire Bowman (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Rapid Fire Bowman (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsRAPIDBOWMAN2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("berapidbowman2_version", PLUGIN_VERSION, "[TF2] Be the Rapid Fire Bowman (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_berapidbowman2", Command_RapidFireBowman2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(RapidFireBowman2SH);
...
{
//	CreateNative("BeRapidFireBowman2_MakeRapidFireBowman2", Native_SetRapidFireBowman2);
//	CreateNative("BeRapidFireBowman2_IsRapidFireBowman2", Native_IsRapidFireBowman2);
...
{
	if (g_bIsRAPIDBOWMAN2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsRAPIDBOWMAN2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(RAPIDBOWMAN2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsRAPIDBOWMAN2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_RapidFireBowman2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeRapidFireBowman2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Rapid Fire Bowman (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Rapid Fire Bowman (Type 2) !");
		PrintToChat(target_list[i], "2. Type 2 is not crit boosted.");
...
{
	if (IsValidClient(client))
			GiveRapidFireBowman2(client);
}
 
stock GiveRapidFireBowman2(client)
{
	if (IsValidClient(client))
	{
		g_bIsRAPIDBOWMAN2[client] = true;
		...
		CreateWeapon(client, "tf_weapon_compound_bow", 56, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsRAPIDBOWMAN2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:RapidFireBowman2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsRAPIDBOWMAN2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetRapidFireBowman2(Handle:plugin, args)
        MakeRapidFireBowman2(GetNativeCell(1));
 
public Native_IsRapidFireBowman2(Handle:plugin, args)
        return g_bIsRAPIDBOWMAN2[GetNativeCell(1)];*/

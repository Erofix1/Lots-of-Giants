...
#define RAPIDSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
public Plugin:myinfo =
{
	...
	description = "Play as the Giant Rapid Fire Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsRAPIDSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("berapidsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Rapid Fire Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_berapidsol", Command_RapidSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(RapidSolSH);
...
{
//	CreateNative("BeRapidSol_MakeRapidSol", Native_SetRapidSol);
//	CreateNative("BeRapidSol_IsRapidSol", Native_IsRapidSol);
...
{
	if (g_bIsRAPIDSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsRAPIDSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(RAPIDSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsRAPIDSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_RapidSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeRapidSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Rapid Fire Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Rapid Fire Soldier !");
		PrintToChat(target_list[i], "2. Your Rocket Launcher can fire endless amount of Rockets !");
...
stock GiveRapidSol(client)
{
	if (IsValidClient(client))
	{
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "clip size upgrade atomic", 96.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsRAPIDSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:RapidSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsRAPIDSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetRapidSol(Handle:plugin, args)
        MakeRapidSol(GetNativeCell(1));
 
public Native_IsRapidSol(Handle:plugin, args)
        return g_bIsRAPIDSOL[GetNativeCell(1)];*/

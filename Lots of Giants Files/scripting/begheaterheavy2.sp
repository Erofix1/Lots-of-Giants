...
#define GHEATHEAVY2      "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Heater Heavy (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Heater Heavy (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGHEATHEAVY2[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("begheaterheavy2_version", PLUGIN_VERSION, "[TF2] Be the Giant Heater Heavy (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begheaterheavy2", Command_GiantHeaterHeavy2, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantHeaterHeavy2SH);
...
{
//	CreateNative("BeGiantHeaterHeavy2_MakeGiantHeaterHeavy2", Native_SetGiantHeaterHeavy2);
//	CreateNative("BeGiantHeaterHeavy2_IsGiantHeaterHeavy2", Native_IsGiantHeaterHeavy2);
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsGHEATHEAVY2[client])
	{
		...
		g_bIsGHEATHEAVY2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHEATHEAVY2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHEATHEAVY2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:Command_GiantHeaterHeavy2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantHeaterHeavy2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Heater Heavy (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Heater Heavy (Type 2) !");
		PrintToChat(target_list[i], "2. Type 2 is a GateBot but Huo-Long Heater deals only 20 percent more damage.");
...
{
	if (IsValidClient(client))
		GiveGiantHeaterHeavy2(client);
}
 
stock GiveGiantHeaterHeavy2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGHEATHEAVY2[client] = true;
		...
		CreateWeapon(client, "tf_weapon_minigun", 811, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 1);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 1060, 10, 6);
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage bonus", 1.2);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHEATHEAVY2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantHeaterHeavy2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHEATHEAVY2[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsGHEATHEAVY2[iClient]) 
	{	
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantHeaterHeavy2(Handle:plugin, args)
        MakeGiantHeaterHeavy2(GetNativeCell(1));
 
public Native_IsGiantHeaterHeavy2(Handle:plugin, args)
        return g_bIsGHEATHEAVY2[GetNativeCell(1)];*/

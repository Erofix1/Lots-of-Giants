...
#define GBURSTSOL2             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Burst Fire Soldier (Type 2)",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Burst Fire Soldier (Type 2) from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGBURSTSOL2[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begburstsol2_version", PLUGIN_VERSION, "[TF2] Be the Giant Burst Fire Soldier (Type 2) version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begburstsol2", Command_GiantBurstFireSol2, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantBurstFireSol2SH);
...
{
//	CreateNative("BeGiantBurstFireSol2_MakeGiantBurstFireSol2", Native_SetGiantBurstFireSol2);
//	CreateNative("BeGiantBurstFireSol2_IsGiantBurstFireSol2", Native_IsGiantBurstFireSol2);
...
{
	if (g_bIsGBURSTSOL2[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGBURSTSOL2[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GBURSTSOL2);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGBURSTSOL2[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantBurstFireSol2(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantBurstFireSol2(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Burst Fire Soldier (Type 2) !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Burst Fire Soldier (Type 2) !");
		PrintToChat(target_list[i], "2. Your Rocket Launcher can fire a big burst of Rockets !");
		PrintToChat(target_list[i], "3. More Damage, Health and a Crit Boost !");
...
{
	if (IsValidClient(client))
			GiveGiantBurstFireSol2(client);
}
 
stock GiveGiantBurstFireSol2(client)
{
	if (IsValidClient(client))
	{
		g_bIsGBURSTSOL2[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGBURSTSOL2[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantBurstFireSol2SH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGBURSTSOL2[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantBurstFireSol2(Handle:plugin, args)
        MakeGiantBurstFireSol2(GetNativeCell(1));
 
public Native_IsGiantBurstFireSol2(Handle:plugin, args)
        return g_bIsGBURSTSOL2[GetNativeCell(1)];*/

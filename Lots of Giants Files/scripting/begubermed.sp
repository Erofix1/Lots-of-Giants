...
#define GUBERMEDIC             "models/bots/medic/bot_medic.mdl"
...
{
	name = "[TF2] Be the Giant Uber Medic",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Uber Medic from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGUBERMEDIC[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begubermed_version", PLUGIN_VERSION, "[TF2] Be the Giant Uber Medic version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begubermed", Command_GiantUberMedic, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantUberMedicSH);
...
{
//	CreateNative("BeGiantUberMedic_MakeGiantUberMedic", Native_SetGiantUberMedic);
//	CreateNative("BeGiantUberMedic_IsGiantUberMedic", Native_IsGiantUberMedic);
...
{
	if (g_bIsGUBERMEDIC[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGUBERMEDIC[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GUBERMEDIC);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGUBERMEDIC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantUberMedic(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
			GiveGiantUberMedic(client);
}
 
stock GiveGiantUberMedic(client)
{
	if (IsValidClient(client))
	{
		g_bIsGUBERMEDIC[client] = true;
		...
		CreateWeapon(client, "tf_weapon_medigun", 29, 6, 1, 2, 0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGUBERMEDIC[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
...
public Action:GiantUberMedicSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGUBERMEDIC[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantUberMedic(Handle:plugin, args)
        MakeGiantUberMedic(GetNativeCell(1));
 
public Native_IsGiantUberMedic(Handle:plugin, args)
        return g_bIsGUBERMEDIC[GetNativeCell(1)];*/

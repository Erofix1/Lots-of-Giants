...
#define GCONCHSOL             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
{
	name = "[TF2] Be the Giant Concheror Soldier",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Giant Concheror Soldier from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGCONCHSOL[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begconchsol_version", PLUGIN_VERSION, "[TF2] Be the Giant Concheror Soldier version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begconchsol", Command_GiantConcherorSol, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantConcherorSolSH);
...
{
//	CreateNative("BeGiantConcherorSol_MakeGiantConcherorSol", Native_SetGiantConcherorSol);
//	CreateNative("BeGiantConcherorSol_IsGiantConcherorSol", Native_IsGiantConcherorSol);
...
{
	if (g_bIsGCONCHSOL[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGCONCHSOL[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GCONCHSOL);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGCONCHSOL[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantConcherorSol(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantConcherorSol(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Concheror Soldier !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Concheror Soldier !");
		PrintToChat(target_list[i], "2. Your Concheror starts with Full RAGE and lasts much longer than normal.");
...
{
	if (IsValidClient(client))
			GiveGiantConcherorSol(client);
}
 
stock GiveGiantConcherorSol(client)
{
	if (IsValidClient(client))
	{
		g_bIsGCONCHSOL[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGCONCHSOL[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
...
public Action:GiantConcherorSolSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGCONCHSOL[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantConcherorSol(Handle:plugin, args)
        MakeGiantConcherorSol(GetNativeCell(1));
 
public Native_IsGiantConcherorSol(Handle:plugin, args)
        return g_bIsGCONCHSOL[GetNativeCell(1)];*/

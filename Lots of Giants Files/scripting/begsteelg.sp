...
#define GSTEELG             "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Giant Steel Gauntlet",
	author = "Erofix using the code and getting help from: HiGPS and his Friends, Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Steel Gauntlet from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsGSTEELG[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("begsteelg_version", PLUGIN_VERSION, "[TF2] Be the Giant Steel Gauntlet version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_begsteelg", Command_GiantSteelGauntlet, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(GiantSteelGauntletSH);
...
{
//	CreateNative("BeGiantSteelGauntlet_MakeGiantSteelGauntlet", Native_SetGiantSteelGauntlet);
//	CreateNative("BeGiantSteelGauntlet_IsGiantSteelGauntlet", Native_IsGiantSteelGauntlet);
...
{
	if (g_bIsGSTEELG[client])
	{
		StopSound(client, SNDCHAN_AUTO, LOOP);
		g_bIsGSTEELG[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GSTEELG);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGSTEELG[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_GiantSteelGauntlet(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeGiantSteelGauntlet(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Giant Steel Gauntlet !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Giant Steel Gauntlet !");
		PrintToChat(target_list[i], "2. Melee only.");
		PrintToChat(target_list[i], "3. You have more health and deal more damage but with slower punch speed.");
...
{
	if (IsValidClient(client))
			GiveGiantSteelGauntlet(client);
}
 
stock GiveGiantSteelGauntlet(client)
{
	if (IsValidClient(client))
	{
		g_bIsGSTEELG[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGSTEELG[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:GiantSteelGauntletSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGSTEELG[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantSteelGauntlet(Handle:plugin, args)
        MakeGiantSteelGauntlet(GetNativeCell(1));
 
public Native_IsGiantSteelGauntlet(Handle:plugin, args)
        return g_bIsGSTEELG[GetNativeCell(1)];*/

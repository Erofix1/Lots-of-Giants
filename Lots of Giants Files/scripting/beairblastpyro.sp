...
#define AIRPYRO		"models/bots/pyro_boss/bot_pyro_boss.mdl"
...
{
	name = "[TF2] Be the Giant Airblast Pyro",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Unused Giant Airblast Pyro from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}

new Handle:g_hEquipWearable;
new bool:g_bIsAIRPYRO[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("beairblastpyro_version", PLUGIN_VERSION, "[TF2] Be the Giant Airblast Pyro version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
	
	RegAdminCmd("sm_beairblastpyro", Command_AirBlastPyro, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(AirBlastPyroSH);
...
{
//	CreateNative("BeAirBlastPyro_MakeAirBlastPyro", Native_SetAirBlastPyro);
//	CreateNative("BeAirBlastPyro_IsAirBlastPyro", Native_IsAirBlastPyro);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsAIRPYRO[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_AirBlastPyro(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
{
	if (IsValidClient(client))
		GiveAirBlastPyro(client);
}

stock GiveAirBlastPyro(client)
{
	if (IsValidClient(client))
	{
		g_bIsAIRPYRO[client] = true;
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "damage penalty", -0.0);
			TF2Attrib_SetByName(Weapon1, "fire rate bonus", 1.0);
			TF2Attrib_SetByName(Weapon1, "airblast pushback scale", 5.0);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsAIRPYRO[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon1 = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
		TF2Attrib_RemoveByName(Weapon1, "damage penalty");
		TF2Attrib_RemoveByName(Weapon1, "fire rate bonus");
		TF2Attrib_RemoveByName(Weapon1, "airblast pushback scale");
		TF2Attrib_RemoveByName(Weapon1, "maxammo primary increased");
		TF2Attrib_RemoveByName(Weapon1, "killstreak tier");
	}
}

public Action:AirBlastPyroSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsAIRPYRO[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsAIRPYRO[iClient]) 
	{	
...
public Native_SetAirBlastPyro(Handle:plugin, args)
	MakeAirBlastPyro(GetNativeCell(1));

public Native_IsAirBlastPyro(Handle:plugin, args)
	return g_bIsAIRPYRO[GetNativeCell(1)];

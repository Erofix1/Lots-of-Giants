...
#define SLWC             "models/bots/heavy_boss/bot_heavy_boss.mdl"
...
{
	name = "[TF2] Be the Super Lightweight Champ",
...
new bool:g_bIsSLWC[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beslwc_version", PLUGIN_VERSION, "[TF2] Be the Super Lightweight Champ version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beslwc", Command_SuperLightweightChamp, ADMFLAG_ROOT, "It's a good time to run");
	AddNormalSoundHook(SuperLightweightChampSH);
...
{
//	CreateNative("BeSuperLightweightChamp_MakeSuperLightweightChamp", Native_SetSuperLightweightChamp);
//	CreateNative("BeSuperLightweightChamp_IsSuperLightweightChamp", Native_IsSuperLightweightChamp);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsSLWC[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
...
public Action:Command_SuperLightweightChamp(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		MakeSuperLightweightChamp(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" turned \"%L\" into Super Lightweight Champ !", client, target_list[i]);
		PrintToChat(target_list[i], "1. You are now Super Lightweight Champ !");
		PrintToChat(target_list[i], "2. Lightweight Champ uses the pre-Jungle Inferno G.R.U. .");
...
MakeSuperLightweightChamp(client)
{
...
{
	if (IsValidClient(client))
			GiveSuperLightweightChamp(client);
}
 
stock GiveSuperLightweightChamp(client)
{
	if (IsValidClient(client))
	{
		g_bIsSLWC[client] = true;
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsSLWC[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon3 = GetPlayerWeaponSlot(client, TFWeaponSlot_Melee);
...
public Action:SuperLightweightChampSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsSLWC[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetSuperLightweightChamp(Handle:plugin, args)
        MakeSuperLightweightChamp(GetNativeCell(1));
 
public Native_IsSuperLightweightChamp(Handle:plugin, args)
        return g_bIsSLWC[GetNativeCell(1)];*/

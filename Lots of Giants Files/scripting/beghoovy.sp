...
#define GHOOVY      "models/bots/heavy_boss/bot_heavy_boss.mdl"
#define SPAWN   "#mvm/giant_heavy/giant_heavy_entrance.wav"
#define DEATH   "mvm/sentrybuster/mvm_sentrybuster_explode.wav"

public Plugin:myinfo =
{
	name = "[TF2] Be the Giant Hoovy",
	author = "Erofix using the code from: Pelipoika, PC Gamer, Jaster and StormishJustice",
	description = "Play as the Custom Giant Hoovy from MvM",
	version = PLUGIN_VERSION,
	url = "www.sourcemod.com"
}
 
new Handle:g_hEquipWearable;
new bool:g_bIsGHOOVY[MAXPLAYERS + 1];
 
public OnPluginStart()
{
	LoadTranslations("common.phrases");
	CreateConVar("beghoovy_version", PLUGIN_VERSION, "[TF2] Be the Giant Hoovy version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
   
	RegAdminCmd("sm_beghoovy", Command_GiantHoovy, ADMFLAG_ROOT, "It's a good time to run");
	
	AddNormalSoundHook(GiantHoovySH);
...
{
//	CreateNative("BeGiantHoovy_MakeGiantHoovy", Native_SetGiantHoovy);
//	CreateNative("BeGiantHoovy_IsGiantHoovy", Native_IsGiantHoovy);
...
{
	if (g_bIsGHOOVY[client])
	{
		g_bIsGHOOVY[client] = false;
	}
}
 
public OnMapStart()
{
	PrecacheModel(GHOOVY);
	PrecacheSound(SPAWN);
	PrecacheSound(DEATH);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsGHOOVY[client])
		{	
			TF2Attrib_RemoveAll(client);
...
public Action:Command_GiantHoovy(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
...
MakeGiantHoovy(client)
{
	...
	TF2Attrib_SetByName(client, "max health additive bonus", 2200.0);
...
{
	if (IsValidClient(client))
		GiveGiantHoovy(client);
}
 
stock GiveGiantHoovy(client)
{
	if (IsValidClient(client))
	{
		g_bIsGHOOVY[client] = true;
		...
		CreateWeapon(client, "tf_weapon_lunchbox", 863, 6, 1, 2, 0);
		TF2_RemoveWeaponSlot(client, 2);
		
		CreateHat(client, 30003, 10, 6);
...
public player_inv(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	if (g_bIsGHOOVY[client] && IsValidClient(client))
	{
		TF2_RemoveAllWearables(client);
		int Weapon2 = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
		TF2Attrib_RemoveByName(Weapon2, "mult_item_meter_charge_rate");
	}
} 

public Action:GiantHoovySH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	if (!IsValidClient(entity)) return Plugin_Continue;
	if (!g_bIsGHOOVY[entity]) return Plugin_Continue;
...
stock TF2_GetNameOfClass(TFClassType:class, String:name[], maxlen)
{
 ...
 /*
public Native_SetGiantHoovy(Handle:plugin, args)
        MakeGiantHoovy(GetNativeCell(1));
 
public Native_IsGiantHoovy(Handle:plugin, args)
        return g_bIsGHOOVY[GetNativeCell(1)];*/
